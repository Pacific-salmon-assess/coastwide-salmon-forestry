...
$log(R_{t,r}/S_{t,r}) = \alpha_r - log(1+(e^{\alpha_t+\alpha_r})/R_{k,r})*S_{t,r}) + \beta_{for,r}*X{t,r} + \epsilon_{t,r}$

$log(R_{t,r}/S_{t,r}) = \alpha_r - log(1+(e^{\alpha_t+\alpha_r})/R_{k,r})*S_{t,r}) + \beta_{for,r}*X{t,r} + \beta_{npgo,r}*Y{t} + \beta_{sst,r}*Z{t,r} + \epsilon_{t,r}$

Ricker:

$log(R_{t,r}/S_{t,r}) = \alpha_t + \alpha_r - S_{t,r}/Smax_r + \beta_{for,r}*X{t,r} + \epsilon_{t,r}$

$log(R_{t,r}/S_{t,r}) = \alpha_r - S_{t,r}/Smax_r + \beta_{for,r}*X{t,r} + \epsilon_{t,r}$

$log(R_{t,r}/S_{t,r}) = \alpha_r - S_{t,r}/Smax_r + \beta_{for,r}*X{t,r} + \beta_{npgo,r}*Y{t} + \beta_{sst,r}*Z{t,r} + \epsilon_{t,r}$
...
plot_recruit_spawner <- function(data, species = "chum", river, posterior, posterior_bh){
  ...
  river_data <- df %>% filter(River_n == river)
    ...
    recruits_predicted <- exp(apply((matrix(posterior_rv_alpha_j[,1], ncol = length(spawners_predicted), nrow = length(posterior_rv_alpha_j[,1])) - 
                                       as.matrix(1/posterior_rv_S_max)%*%spawners_predicted), 2, median))*spawners_predicted
    ...
    recruits_bh_predicted <- exp(apply((matrix(posterior_bh_rv_alpha_j[,1], ncol = length(spawners_predicted), nrow = length(posterior_bh_rv_alpha_j[,1])) - log(1+as.matrix(exp(posterior_bh_rv_alpha_j[,1])/posterior_bh_rv_Rk)%*%spawners_predicted)), 2, median))*spawners_predicted
      
    recruits_bh_predicted_lower <- exp(apply((matrix(posterior_bh_rv_alpha_j[,1], ncol = length(spawners_predicted), nrow = length(posterior_bh_rv_alpha_j[,1])) - log(1+as.matrix(exp(posterior_bh_rv_alpha_j[,1])/posterior_bh_rv_Rk)%*%spawners_predicted)), 2, quantile, c(0.025)))*spawners_predicted
                                       
                                    
    recruits_bh_predicted_upper <- exp(apply((matrix(posterior_bh_rv_alpha_j[,1], ncol = length(spawners_predicted), nrow = length(posterior_bh_rv_alpha_j[,1])) - log(1+as.matrix(exp(posterior_bh_rv_alpha_j[,1])/posterior_bh_rv_Rk)%*%spawners_predicted)), 2, quantile, c(0.975)))*spawners_predicted
    ...
    p1 <- ggplot() +
      ...
      geom_line(data = prediction_df, aes(x = spawners, y = recruits), color = "black", size = 1) +
      geom_ribbon(data = prediction_df, aes(x = spawners, ymin = recruits_lower, ymax = recruits_upper), fill = "gray", alpha = 0.5) +
      geom_line(data = prediction_df, aes(x = spawners, y = recruits_low_cpd), color = '#35978f', size = 1) +
      geom_line(data = prediction_df, aes(x = spawners, y = recruits_high_cpd), color = '#bf812d', size = 1) +
      #make y log scale
      # scale_y_log10() +
      ...
      scale_color_gradient2(name = 'Max CPD\nin River (%)',
                                        low = '#35978f', mid = 'gray', high = '#bf812d', midpoint = 50)+
      theme_classic() +
      theme(legend.position = "right",
            legend.key.width = unit(0.5, "cm"),
    ...
    p2 <- ggplot(river_data) +
      ...
      geom_line(data = prediction_bh_df, aes(x = spawners, y = recruits), color = "black", size = 1) +
      geom_ribbon(data = prediction_bh_df, aes(x = spawners, ymin = recruits_lower, ymax = recruits_upper), fill = "gray", alpha = 0.5) +
      #make y log scale
      # scale_y_log10() +
      
      geom_line(data = prediction_bh_df, aes(x = spawners, y = recruits_low_cpd), color = '#35978f', size = 1) +
      geom_line(data = prediction_bh_df, aes(x = spawners, y = recruits_high_cpd), color = '#bf812d', size = 1) +
    ...
    p3 <- ggplot(river_data) + 
      geom_point(aes(x = Spawners, y = log(Recruits/Spawners), color = disturbedarea_prct_cs), alpha = 0.2, size = 2) +
      geom_line(data = prediction_df, aes(x = spawners, y = log_RS), color = "black", size = 1) +
      ...
      geom_line(data = prediction_df, aes(x = spawners, y = log(recruits_predicted_low_cpd/spawners_predicted)), color = '#35978f', size = 1) +
      geom_line(data = prediction_df, aes(x = spawners, y = log(recruits_predicted_high_cpd/spawners_predicted)), color = '#bf812d', size = 1) +
      labs(#title = "Ricker model with CPD, NPGO, ERSST", 
           x = "Spawners", y = "log(Recruits/Spawners)") +
      scale_color_gradient2(name = 'Max CPD\nin River (%)',
                                        low = '#35978f', mid = 'gray', high = '#bf812d', midpoint = 50)+
      theme_classic() +
      theme(legend.position = "right",
    ...
    p4 <- ggplot(river_data) + 
      geom_point(aes(x = Spawners, y = log(Recruits/Spawners), color = disturbedarea_prct_cs), alpha = 0.2, size = 2) +
      geom_line(data = prediction_bh_df, aes(x = spawners, y = log_RS), color = "black", size = 1) +
      ...
      geom_line(data = prediction_bh_df, aes(x = spawners, y = log(recruits_bh_predicted_low_cpd/spawners_predicted)), color = '#35978f', size = 1) +
      geom_line(data = prediction_bh_df, aes(x = spawners, y = log(recruits_bh_predicted_high_cpd/spawners_predicted)), color = '#bf812d', size = 1) +
...
for(river in rivers){
  ...
  plot_recruit_spawner(data = df, species = species, river = river, posterior = posterior, posterior_bh = posterior_bh)
}

plot_recruit_spawner(river = 310, posterior = posterior, posterior_bh = posterior_bh)
