...
The following figures are observations and predictions for spawner-recruit and log(recruits/spawner) relationships for each river. The observations and predictions are coloured based on CPD levels in that river. The black line is the prediction associated with mid forest disturbance levels, and the shaded area represents the 95% credible interval. Note that some rivers have experienced small ranges of forest disturbance. 
...
## Spawner Recruit fits for models with forestry effects on K

```{r, spawner-recruit-k, fig.width = 10, fig.height = 8}


plot_recruit_spawner_river_k <- function(data, species = "chum", river, river_data,  posterior, posterior_bh){
  ...
  if(species == "chum"){
    ...
    posterior_bh_rv_k <- posterior_bh %>%
      select(starts_with('Rk')) %>%
      select(ends_with(paste0("[",river,"]")))
    ...
    recruits_predicted <- dexp(apply(matrix(posterior_rv_alpha_j[,1], ncol = length(spawners_predicted), nrow = length(posterior_rv_alpha_j[,1]))*(1 -  k_rv_inv%*%spawners_predicted_scaled), 2, median))*spawners_predicted
