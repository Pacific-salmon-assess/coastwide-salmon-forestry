...
```{r, sr time-series chm ric, fig.width=12,fig.height=16, results="asis"}
pk10r$River2=paste(pk10r$River,pk10r$Broodline,sep='_')
pk10r$River2.n=as.numeric(factor(pk10r$River2))

d1eca=read.csv(here('stan models','outs','posterior','bh_pk_eca.csv'))
d1cpd=read.csv(here('stan models','outs','posterior','bh_pk_cpd.csv'),check.names=F)
r1eca=read.csv(here('stan models','outs','posterior','ric_pk_eca.csv'),check.names=F)
r1cpd=read.csv(here('stan models','outs','posterior','ric_pk_cpd.csv'),check.names=F)
...
for(c in 1:length(unique(ch20r$River))){
  ...
  Rk_j_o=d1eca[,colnames(d1eca)== paste('Rk.',unique(rv_e$River2.n),'.',sep='')]
  ...
  #Odd
    plot(Recruits~Spawners,data=rv_o,xlim=c(0,max(rv_e$Spawners)),ylim=c(0,max(rv_e$Recruits)),bty='l',type='n',main='Odd - Ricker (ECA)')
  ...
  #CPD
  plot(Recruits~Spawners,data=rv_e,xlim=c(0,max(rv_e$Spawners)),ylim=c(0,max(rv_e$Recruits)),bty='l',type='n',main=paste(unique(rv_e$River),'(CPD)',sep=' '))
  
   alpha_t=apply(d1cpd[,grepl('alpha_t.1',colnames(d1cpd))==T],1,median)
  alpha_j=d1cpd[,colnames(d1cpd)== paste('alpha_j[',c,']',sep='')]
  Rk_j=d1cpd[,colnames(d1cpd)== paste('Rk[',c,']',sep='')]
  b_ECA=d1cpd[,colnames(d1cpd)==paste('b_for_rv[',c,']',sep='')]                                                                          
  s_pred=seq(0,max(rv$Spawners),length.out=100)
