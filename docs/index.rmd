...
Ricker formulation (with River-level varying effects for ECA):
...
This plot shows density of effect sizes among Rivers and the red line indicates the global effect:
...
The realized function for ECA on productivity (in BH form):
...
$log(R_{t,r}/S_{t,r}) = \alpha_r - \beta_r*S_{t,r} + \beta_{eca,c}*ECA_{t,r} + \beta_{eca_x_area,c}*ECA_{t,r}*Area_{r} + w_{t,r}$
...
An important point to consider is that the range of observed ECAs can vary substantially by rivers, and 
...
text(x=0.7,y=0.1,'1km2',col='darkred')
text(x=0.7,y=-0.2,'25km2',col='goldenrod')
text(x=0.7,y=-0.7,'5800km2',col='navy')
...
To explore if there were any signs of strong non-linear patterns in how ECA affects forestry we implemented an approach equivalent to a Generalized Additive Model (GAM). We use a matrix of B-splines with 6 knots that subdivide the predictor (ECA), the resulting estimated effect of ECA at any given point is the sum of these smoothed functions. The global function of ECA effects on productivity fit to all rivers/CUs"
...
# ECA and effect size time-series - Beverton Holt

Overall distribution of ECA among all observations:

```{r, eca histogram, fig.width=8,fig.height=6}
hist(ch20r$ECA_age_proxy_forested_only,breaks=30,main='ECA for all observations',xlab='ECA')

```

River-level ECA and effect on productivity through time by CU (BH model):

```{r, eca time-series, fig.width=8,fig.height=12, results="asis"}
...
# River-level Spawner-Recruit fits - BH
```{r, sr time-series, fig.width=12,fig.height=8, results="asis"}
ch20r$j=as.numeric(factor(ch20r$River))
cus=distinct(ch20r,River,.keep_all=T)
cus=cus[order(cus$CU),]
...
for(c in 1:22){
  ...
  cat("## ", c, "\n")
  #CU header
  cu=subset(ch20r,CU==levels(factor(ch20r$CU))[c])
  
  for(j in 1:length(unique(cu$River))){
  rv.id=unique(cu$River[j])
  rv=subset(ch20r,River==rv.id)
  ...
  alpha_j=d_bh1[,colnames(d_bh1) %in% paste('alpha_j[',unique(rv$j),']',sep='')]
  Rk_j=d_bh1[,colnames(d_bh1) %in% paste('Rk[',unique(rv$j),']',sep='')]
  b_ECA=d_bh1[,colnames(d_bh1) %in% paste('b_ECA_cu[',c,']',sep='')]
  ...
  text(x=max(s_pred)*0.9,y=max(RS_pred_eca_l)*1.08,col='darkgreen','min. obs. ECA')
  lines(RS_pred_eca_u~s_pred,col='darkorange',lwd=3)
  text(x=max(s_pred)*0.9,y=max(RS_pred_eca_u)*0.92,col='darkorange','max. obs. ECA')
  
  points(Recruits~Spawners,data=rv,bg=adjustcolor('darkgray',alpha.f=0.8),pch=21) 
  ...
  alpha_j=d_ric1[,colnames(d_ric1) %in% paste('alpha_j[',unique(rv$j),']',sep='')]
  Smax=d_ric1[,colnames(d_ric1) %in% paste('Smax[',unique(rv$j),']',sep='')]
  b_ECA=d_ric1[,colnames(d_ric1) %in% paste('b_ECA_cu[',c,']',sep='')]
  s_pred=seq(0,max(rv$Spawners),length.out=100)
  ...
  lines(RS_pred_u~s_pred,lwd=1,lty=5)
  lines(RS_pred_eca_l~s_pred,col='darkgreen',lwd=3)
  text(x=max(s_pred)*0.9,y=RS_pred_eca_l[100]*1.15,col='darkgreen','min. obs. ECA')
  lines(RS_pred_eca_u~s_pred,col='darkorange',lwd=3)
  text(x=max(s_pred)*0.9,y=RS_pred_eca_u[100]*0.92,col='darkorange','max. obs. ECA')
