...
for(c in 1:length(unique(pk10r$River))){
  ...
  #Even
  if(nrow(rv_e)>0){
   plot(Recruits~Spawners,data=rv_e,xlim=c(0,max(rv_e$Spawners)),ylim=c(0,max(rv_e$Recruits)),bty='l',type='n',main=paste(unique(rv_e$River),'Even (ECA)',sep=' '))
  ...
  b_ECA=d1eca[,colnames(d1eca)== paste('b_for_rv.',c,'.',sep='')]                           
   plot(Recruits~Spawners,data=rv_o,xlim=c(0,max(rv_o$Spawners)),ylim=c(0,max(rv_o$Recruits)),bty='l',type='n',main=paste(unique(rv_o$River),'Odd (ECA)',sep=' '))
  ...
  #Ricker
 if(nrow(rv_e)>0){ 
   plot(Recruits~Spawners,data=rv_e,xlim=c(0,max(rv_e$Spawners)),ylim=c(0,max(rv_e$Recruits)),bty='l',type='n',main='Even - Ricker (ECA)')
  ...
  b_ECA=r1eca[,colnames(r1eca)== paste('b_for_rv.',c,'.',sep='')]                           
    ...
    plot(Recruits~Spawners,data=rv_o,xlim=c(0,max(rv_o$Spawners)),ylim=c(0,max(rv_o$Recruits)),bty='l',type='n',main='Odd - Ricker (ECA)')
  ...
  #even
  if(nrow(rv_e)>0){
  plot(Recruits~Spawners,data=rv_e,xlim=c(0,max(rv_e$Spawners)),ylim=c(0,max(rv_e$Recruits)),bty='l',type='n',main=paste(unique(rv_e$River),'Even (CPD)',sep=' '))
 ...
 Rk_j_o=d1cpd[,colnames(d1cpd)== paste('Rk.',unique(rv_o$River2.n),'.',sep='')]
   ...
   plot(Recruits~Spawners,data=rv_o,xlim=c(0,max(rv_o$Spawners)),ylim=c(0,max(rv_o$Recruits)),bty='l',type='n',main=paste(unique(rv_o$River),'Even (CPD)',sep=' '))
    RS_pred_m=exp(apply(matrix(alpha_j_o+alpha_t_o,ncol=length(s_pred),nrow=length(alpha_j_o))-log(1+as.matrix(exp(alpha_j_o+alpha_t_o)/Rk_j_o)%*%s_pred),2,median))*s_pred
  RS_pred_l=exp(apply(matrix(alpha_j_o+alpha_t_o,ncol=length(s_pred),nrow=length(alpha_j_o))-log(1+as.matrix(exp(alpha_j_o+alpha_t_o)/Rk_j_o)%*%s_pred),2,quantile,0.05))*s_pred
  RS_pred_u=exp(apply(matrix(alpha_j_o+alpha_t_o,ncol=length(s_pred),nrow=length(alpha_j_o))-log(1+as.matrix(exp(alpha_j_o+alpha_t_o)/Rk_j_o)%*%s_pred),2,quantile,0.95))*s_pred
