...
hist(d_chm_eca2[,2],breaks=30,main='ECA (no SC)',xlab='effect size')
hist(d_chm_cpd2[,2],breaks=30,main='CPD (no SC)',xlab='effect size')
...
for(c in 1:length(unique(pk10r$River))){
 ...
 if(nrow(rv_e)>0){ 
  ...
  points(Recruits~Spawners,data=rv_e,bg=col_points2,pch=21,cex=1.5) 
 }
  #Odd
  if(nrow(rv_o)>0){
      smax_j_o=d1eca[,colnames(r1eca)== paste('S_max.',unique(rv_o$River2.n),'.',sep='')]
  ...
  points(Recruits~Spawners,data=rv_o,bg=col_points2,pch=21,cex=1.5) 
  }
