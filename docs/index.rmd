...
d1eca=read.csv(here('stan models','outs','fits','posterior','fit1bh_chm_eca_linear.csv'))
...
d1cpd=read.csv(here('stan models','outs','fits','posterior','fit1bh_chm_cpd_linear.csv'))
...
pRSe=exp(matrix(d1eca[,colnames(d1eca)=='alpha_0'],ncol=100,nrow=nrow(d1eca))+as.matrix(d1eca[,2])%*%x_n)/exp(matrix(d1eca[,colnames(d1eca)=='alpha_0'],ncol=100,nrow=nrow(d1eca))+matrix(as.matrix(d1eca[,2])%*%x_n[1],ncol=100,nrow=nrow(d1eca)))
...
pRSc=exp(matrix(d1cpd[,colnames(d1cpd)=='alpha_0'],ncol=100,nrow=nrow(d1cpd))+as.matrix(d1cpd[,2])%*%x_n)/exp(matrix(d1cpd[,colnames(d1cpd)=='alpha_0'],ncol=100,nrow=nrow(d1cpd))+matrix(as.matrix(d1cpd[,2])%*%x_n[1],ncol=100,nrow=nrow(d1cpd)))
...
plot(m_pred*100~cpd_n,bty='l',type='n',ylab='Change in intrinsic productivity (recruits/spawner, %)',xlab='Cumulative disturbed area (%)',ylim=c(-90,100),col='darkred',lwd=3,yaxt='n')
axis(side=2,at=seq(-90,100,by=10))
abline(h=100,lwd=0.5,lty=5)
for(c in 1:22){
...
mtext(side=2,'Posterior density')
...
pRS=exp(matrix(d1eca[,colnames(d1eca)=='alpha_0'],ncol=100,nrow=nrow(d1eca))+as.matrix(d1eca[,2])%*%x_n)/exp(matrix(d1eca[,colnames(d1eca)=='alpha_0'],ncol=100,nrow=nrow(d1eca))+matrix(d1eca[,2]%*%x_n[1],ncol=100,nrow=nrow(d1eca)))
m_pred=apply(pRS,2,median)
cols=RColorBrewer::brewer.pal(7,'Blues');
cols=cols[-1] #remove first colour - too pale to see
br1=quantile(cu.nrv,seq(0,1,by=0.2))
...
plot(m_pred~cpd_n,bty='l',type='n',ylab='change in log(R/S)',xlab='Cumulative % disturbed',ylim=c(-2,2),col='darkred',lwd=3,main='logit CPD')
for(j in 3:ncol(d_logit)){
...
for(c in 1:length(unique(ch20r$River))){
 ...
 realized_eca_m=apply(realized_eca,2,median)
   ...
   realized_cpd=dbh1_chm_cpd[,colnames(dbh1_chm_cpd) %in% paste('b_ECA_cu[',unique(rv$cuid),']',sep='')]%*%rv$logit.pdisturb.std
