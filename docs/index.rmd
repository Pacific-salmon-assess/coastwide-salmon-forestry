...
Stock-specific capacity parameter ($Smax_r$) is estimated independently at the river level, but with informative priors based on observed spawner counts ($S_t,r$):

$Smax_r \sim lognormal(log(0.5*max(S_{r,r})),log(max(S_{t,r}))$

For Pinks, the model has two separate coastwide productivity trajectories for the distinct odd/even broodlines, $bl$, but the effects of forestry metrics are shared between broodlines in the same river:

$log(R_{t,r,bl}/S_{t,r,bl}) = \alpha_{t,bl} + \alpha_{r,bl} - log(1+(e^{\alpha_{t,bl}+\alpha_j})/R_{k,r,bl})*S_{t,r,bl}) + \beta_{for,r}*X{t,r,bl} + \epsilon_{t,r,bl}$

$\alpha_{t=1,bl='even'} \sim N(1.5,2.5)$
$\alpha_{t=1,bl='odd'} \sim N(1.5,2.5)$

$\alpha_{t=2,..,T,bl='even'} = \alpha_{t-1,bl='even'} + w_{t,1}$
$\alpha_{t=2,..,T,bl='odd'} = \alpha_{t-1,bl='odd'} + w_{t,2}$

$w_t,1 \sim N(0,\sigma_{w,even})$
$w_t,2 \sim N(0,\sigma_{w,odd})$

$\sigma_{w,even} \sim N[0,1)$
$\sigma_{w,odd} \sim N[0,1)$
...
# Static vs. time-varying across model forms - Chum
```{r, chm tv/st}
bh_chm_eca=read.csv(here('stan models','outs','posterior','bh_chm_eca.csv'),check.names=F)
bh_chm_eca_st=read.csv(here('stan models','outs','posterior','bh_chm_eca_st.csv'),check.names=F)

ric_chm_eca=read.csv(here('stan models','outs','posterior','ric_chm_eca_ac.csv'),check.names=F)
ric_chm_eca_st=read.csv(here('stan models','outs','posterior','ric_chm_eca_st.csv'),check.names=F)

csh_chm_eca=read.csv(here('stan models','outs','posterior','cush_chm_eca.csv'),check.names=F)
csh_chm_eca_st=read.csv(here('stan models','outs','posterior','cush_chm_eca_st.csv'),check.names=F)

bh_chm_cpd=read.csv(here('stan models','outs','posterior','bh_chm_cpd.csv'),check.names=F)
bh_chm_cpd_st=read.csv(here('stan models','outs','posterior','bh_chm_cpd_st.csv'),check.names=F)

ric_chm_cpd=read.csv(here('stan models','outs','posterior','ric_chm_cpd.csv'),check.names=F)
ric_chm_cpd_st=read.csv(here('stan models','outs','posterior','ric_chm_cpd_st.csv'),check.names=F)

csh_chm_cpd=read.csv(here('stan models','outs','posterior','cush_chm_cpd.csv'),check.names=F)
csh_chm_cpd_st=read.csv(here('stan models','outs','posterior','cush_chm_cpd_st.csv'),check.names=F)


par(mfrow=c(3,2))
plot(c(0,10)~c(-1.5,1.5),bty='l',type='n',ylab='Posterior density',main='ECA effect sizes (time-varying, Ricker)',ylim=c(0,10),yaxt='n',xlab='Standardized coefficients')
d0eca=density(ric_chm_eca[,2],bw=0.02)
for(j in 3:24){
  d=density(ric_chm_eca[,j],bw=0.02)
  lines(d$y~d$x,col=adjustcolor('darkgray',alpha.f=0.6))
}
lines(d0eca$y~d0eca$x,col='darkred',lwd=3)
abline(v=0,lwd=2)

plot(c(0,10)~c(-1.5,1.5),bty='l',type='n',ylab='Posterior density',main='ECA effect sizes (static, Ricker)',ylim=c(0,10),yaxt='n',xlab='Standardized coefficients')
d0eca=density(ric_chm_eca_st[,2],bw=0.02)
for(j in 3:24){
  d=density(ric_chm_eca_st[,j],bw=0.02)
  lines(d$y~d$x,col=adjustcolor('darkgray',alpha.f=0.6))
}
lines(d0eca$y~d0eca$x,col='darkred',lwd=3)
abline(v=0,lwd=2)
...
#ricker - cpd
plot(c(0,10)~c(-1.5,1.5),bty='l',type='n',ylab='Posterior density',main='CPD effect sizes (time-varying, Ricker)',ylim=c(0,10),yaxt='n',xlab='Standardized coefficients')
d0cpd=density(ric_chm_cpd[,2],bw=0.02)
for(j in 3:24){
  d=density(ric_chm_cpd[,j],bw=0.02)
  lines(d$y~d$x,col=adjustcolor('darkgray',alpha.f=0.6))
}
lines(d0cpd$y~d0cpd$x,col='darkred',lwd=3)
abline(v=0,lwd=2)

plot(c(0,10)~c(-1.5,1.5),bty='l',type='n',ylab='Posterior density',main='CPD effect sizes (static, Ricker)',ylim=c(0,10),yaxt='n',xlab='Standardized coefficients')
d0cpd=density(ric_chm_cpd_st[,2],bw=0.02)
for(j in 3:24){
  d=density(ric_chm_cpd_st[,j],bw=0.02)
  lines(d$y~d$x,col=adjustcolor('darkgray',alpha.f=0.6))
}
lines(d0cpd$y~d0cpd$x,col='darkred',lwd=3)
abline(v=0,lwd=2)
...
#cushing
plot(c(0,10)~c(-1.5,1.5),bty='l',type='n',ylab='Posterior density',main='CPD effect sizes (time-varying, Cushing)',ylim=c(0,10),yaxt='n',xlab='Standardized coefficients')
d0cpd=density(csh_chm_cpd[,2],bw=0.02)
for(j in 3:24){
  d=density(csh_chm_cpd[,j],bw=0.02)
  lines(d$y~d$x,col=adjustcolor('darkgray',alpha.f=0.6))
}
lines(d0cpd$y~d0cpd$x,col='darkred',lwd=3)
abline(v=0,lwd=2)

plot(c(0,10)~c(-1.5,1.5),bty='l',type='n',ylab='Posterior density',main='CPD effect sizes (static, Cushing)',ylim=c(0,10),yaxt='n',xlab='Standardized coefficients')
d0cpd=density(csh_chm_cpd_st[,2],bw=0.02)
for(j in 3:24){
  d=density(csh_chm_cpd_st[,j],bw=0.02)
  lines(d$y~d$x,col=adjustcolor('darkgray',alpha.f=0.6))
}
lines(d0cpd$y~d0cpd$x,col='darkred',lwd=3)
abline(v=0,lwd=2)
...
# Fig. 3_1. (Beverton-Holt) Global effects of ECA/CPD by species ( conditional on time-varying productivity trends) ####
...
# Fig. 3_2. Global effects of ECA/CPD by species (Ricker, conditional on time-varying productivity trends) ####
...
# Fig. S1. Coastwide productivity trends by species (Beverton-Holt) ####
...
# Fig. S2. Coastwide productivity trends by species (Ricker) ####
...
# Fig. S3. Coastwide productivity trends by species (Cushing) ####

Time-varying coastwide productivity estimates from Cushing spawner-recruit fits:

```{r, fig. Sx1 - ricker,fig.width=8,fig.height=12}
d_chm_cpd=read.csv(here('stan models','outs','posterior','cush_chm_eca.csv'),check.names=F)
d_pk_cpd=read.csv(here('stan models','outs','posterior','cush_pk_cpd.csv'),check.names=F)
...
# Fig. SX3. River-level variance in forestry effects (Beverton-Holt) ####
...
# Fig. SX4. Response to forestry vs. scale of forestry in watershed####

```{r, fig. Sx3 - draft 1,fig.width=8,fig.height=11.50}
chm_rv=data.frame(unique(ch20r$WATERSHED_CDE),unique(ch20r$River))
p_rv=data.frame(unique(pk10r$WATERSHED_CDE),unique(pk10r$River))

p_rv2=chm_rv[chm_rv[,1]%in%p_rv[,1],]
ch_id=match(p_rv2[,2],levels(factor(ch20r$River)))
p_id=match(p_rv2[,2],levels(factor(pk10r$River)))

chm_bfor_cpd2=data.frame(d_chm_cpd[,colnames(d_chm_cpd)%in%paste('b_for_rv[',ch_id,']',sep='')])
p_bfor_cpd2=data.frame(d_pk_cpd[,colnames(d_pk_cpd)%in%paste('b_for_rv[',p_id,']',sep='')])
m_chm_bfor_cpd2=apply(chm_bfor_cpd2,2,median)
m_p_bfor_cpd2=apply(p_bfor_cpd2,2,median)


p_rv2$bfor.chm=m_chm_bfor_cpd2[match(paste('b_for_rv.',ch_id,'.',sep=''),names(m_chm_bfor_cpd2))]
p_rv2$bfor.pk=m_p_bfor_cpd2[match(paste('b_for_rv.',p_id,'.',sep=''),names(m_p_bfor_cpd2))]

plot(p_rv2$bfor.chm~p_rv2$bfor.pk)
...
```{r, south coast, fig.width=11,fig.height=11}
...
d_chm_eca2=read.csv(here('stan models','outs','posterior','bh_chm_eca.csv'),check.names=F)
d_chm_cpd2=read.csv(here('stan models','outs','posterior','bh_chm_cpd.csv'),check.names=F)

par(mfrow=c(2,2))
hist(d_chm_eca[,2],breaks=30,main='ECA',xlab='effect size',xlim=c(-0.5,0.3))
hist(d_chm_cpd[,2],breaks=30,main='CPD',xlab='effect size',xlim=c(-0.5,0.3))
hist(d_chm_eca2[,2],breaks=30,main='ECA (no SC)',xlab='effect size',xlim=c(-0.5,0.3))
hist(d_chm_cpd2[,2],breaks=30,main='CPD (no SC)',xlab='effect size',xlim=c(-0.5,0.3))
...
for(c in 1:length(unique(ch20r$River))){
 ...
 realized_cpd_m_r=apply(realized_cpd_r,2,median)
   ...
   plot(realized_eca_m~rv$BroodYear,bty='l',type='n',ylim=c(min(c(realized_eca_m,realized_cpd_m,realized_cpd_m_r,realized_eca_m_r)),max(c(realized_eca_m,realized_cpd_m,realized_cpd_m_r,realized_eca_m_r))*1.2),ylab='Marginal effect on productivity',xlab='Brood Year')
...
```{r, sr time-series pk, fig.width=12,fig.height=16, results="asis"}
pk10r$River2=paste(pk10r$River,pk10r$Broodline,sep='_')
pk10r$River2.n=as.numeric(factor(pk10r$River2))

d1eca=read.csv(here('stan models','outs','posterior','bh_pk_eca.csv'))
d1cpd=read.csv(here('stan models','outs','posterior','bh_pk_cpd.csv'))
r1eca=read.csv(here('stan models','outs','posterior','ric_pk_eca.csv'))
r1cpd=read.csv(here('stan models','outs','posterior','ric_pk_cpd.csv'))

for(c in 1:length(unique(pk10r$River))){
  cat('\n')  
  cat("### ", levels(factor(pk10r$River))[c], "\n")
  #CU header
  rv=subset(pk10r,River==levels(factor(pk10r$River))[c])
  rv_e=subset(rv,Broodline=='Even')
  rv_o=subset(rv,Broodline=='Odd')
  ...
  #Even
  if(nrow(rv_e)>0){
   plot(Recruits~Spawners,data=rv_e,xlim=c(0,max(rv$Spawners)),ylim=c(0,max(rv$Recruits)),bty='l',type='n',main=paste(unique(rv_e$River),'Even (ECA)',sep=' '))
  
  alpha_t_e=apply(d1eca[,grepl('alpha_t.1',colnames(d1eca))==T],1,median)
  alpha_j_e=d1eca[,colnames(d1eca)== paste('alpha_j.',unique(rv_e$River2.n),'.',sep='')]
 
  
  Rk_j_e=d1eca[,colnames(d1eca)== paste('Rk.',unique(rv_e$River2.n),'.',sep='')]
  
  b_ECA=d1eca[,colnames(d1eca)== paste('b_for_rv.',c,'.',sep='')]                                                                          
  s_pred=seq(0,max(rv_e$Spawners),length.out=100)
  
  RS_pred_m=exp(apply(matrix(alpha_j_e+alpha_t_e,ncol=length(s_pred),nrow=length(alpha_j_e))-log(1+as.matrix(exp(alpha_j_e+alpha_t_e)/Rk_j_e)%*%s_pred),2,median))*s_pred
  RS_pred_l=exp(apply(matrix(alpha_j_e+alpha_t_e,ncol=length(s_pred),nrow=length(alpha_j_e))-log(1+as.matrix(exp(alpha_j_e+alpha_t_e)/Rk_j_e)%*%s_pred),2,quantile,0.05))*s_pred
  RS_pred_u=exp(apply(matrix(alpha_j_e+alpha_t_e,ncol=length(s_pred),nrow=length(alpha_j_e))-log(1+as.matrix(exp(alpha_j_e+alpha_t_e)/Rk_j_e)%*%s_pred),2,quantile,0.95))*s_pred
  
  
  RS_pred_eca_l=exp(apply(matrix(alpha_j_e+alpha_t_e,ncol=length(s_pred),nrow=length(alpha_j_e))-log(1+as.matrix(exp(alpha_j_e+alpha_t_e)/Rk_j_e)%*%s_pred)+matrix(b_ECA*min(rv$sqrt.ECA.std),ncol=length(s_pred),nrow=length(alpha_j_e)),2,median))*s_pred
  
  RS_pred_eca_u=exp(apply(matrix(alpha_j_e+alpha_t_e,ncol=length(s_pred),nrow=length(alpha_j_e))-log(1+as.matrix(exp(alpha_j_e+alpha_t_e)/Rk_j_e)%*%s_pred)+matrix(b_ECA*max(rv$sqrt.ECA.std),ncol=length(s_pred),nrow=length(alpha_j_e+alpha_t_e)),2,median))*s_pred
  ...
  points(Recruits~Spawners,data=rv_e,bg=col_points1,pch=21,cex=1.5) 
}
   #Odd
  if(nrow(rv_o)>0){
  alpha_t_o=apply(d1eca[,grepl('alpha_t.2',colnames(d1eca))==T],1,median)
  alpha_j_o=d1eca[,colnames(d1eca)== paste('alpha_j.',unique(rv_o$River2.n),'.',sep='')]
  Rk_j_o=d1eca[,colnames(d1eca)== paste('Rk.',unique(rv_o$River2.n),'.',sep='')]
  b_ECA=d1eca[,colnames(d1eca)== paste('b_for_rv.',c,'.',sep='')]                           
   plot(Recruits~Spawners,data=rv_o,xlim=c(0,max(rv$Spawners)),ylim=c(0,max(rv$Recruits)),bty='l',type='n',main=paste(unique(rv_o$River),'Odd (ECA)',sep=' '))

  s_pred=seq(0,max(rv_o$Spawners),length.out=100)
  
   RS_pred_m=exp(apply(matrix(alpha_j_o+alpha_t_o,ncol=length(s_pred),nrow=length(alpha_j_o))-log(1+as.matrix(exp(alpha_j_o+alpha_t_o)/Rk_j_o)%*%s_pred),2,median))*s_pred
  RS_pred_l=exp(apply(matrix(alpha_j_o+alpha_t_o,ncol=length(s_pred),nrow=length(alpha_j_o))-log(1+as.matrix(exp(alpha_j_o+alpha_t_o)/Rk_j_o)%*%s_pred),2,quantile,0.05))*s_pred
  RS_pred_u=exp(apply(matrix(alpha_j_o+alpha_t_o,ncol=length(s_pred),nrow=length(alpha_j_o))-log(1+as.matrix(exp(alpha_j_o+alpha_t_o)/Rk_j_o)%*%s_pred),2,quantile,0.95))*s_pred
  
  
  RS_pred_eca_l=exp(apply(matrix(alpha_j_o+alpha_t_o,ncol=length(s_pred),nrow=length(alpha_j_o))-log(1+as.matrix(exp(alpha_j_o+alpha_t_o)/Rk_j_o)%*%s_pred)+matrix(b_ECA*min(rv$sqrt.ECA.std),ncol=length(s_pred),nrow=length(alpha_j_o)),2,median))*s_pred
  
  RS_pred_eca_u=exp(apply(matrix(alpha_j_o+alpha_t_o,ncol=length(s_pred),nrow=length(alpha_j_o))-log(1+as.matrix(exp(alpha_j_o+alpha_t_o)/Rk_j_o)%*%s_pred)+matrix(b_ECA*max(rv$sqrt.ECA.std),ncol=length(s_pred),nrow=length(alpha_j_o+alpha_t_o)),2,median))*s_pred
  ...
  points(Recruits~Spawners,data=rv_o,bg=col_points1,pch=21,cex=1.5) 
}
  #Ricker
 if(nrow(rv_e)>0){ 
   plot(Recruits~Spawners,data=rv_e,xlim=c(0,max(rv$Spawners)),ylim=c(0,max(rv$Recruits)),bty='l',type='n',main='Even - Ricker (ECA)')
  
  smax_j_e=d1eca[,colnames(r1eca)== paste('S_max.',unique(rv_e$River2.n),'.',sep='')]
  alpha_t_e=apply(r1eca[,grepl('alpha_t.1',colnames(r1eca))==T],1,median)
  alpha_j_e=d1eca[,colnames(r1eca)== paste('alpha_j.',unique(rv_e$River2.n),'.',sep='')]

  b_ECA=r1eca[,colnames(r1eca)== paste('b_for_rv.',c,'.',sep='')]                                #even       
    s_pred=seq(0,max(rv_e$Spawners),length.out=100)
    
   RS_pred_m=exp(apply(matrix(alpha_j_e+alpha_t_e,ncol=length(s_pred),nrow=length(alpha_j_e))-as.matrix(1/smax_j_e)%*%s_pred,2,median))*s_pred
  RS_pred_l=exp(apply(matrix(alpha_j_e+alpha_t_e,ncol=length(s_pred),nrow=length(alpha_j_e))-as.matrix(1/smax_j_e)%*%s_pred,2,quantile,0.05))*s_pred
  RS_pred_u=exp(apply(matrix(alpha_j_e+alpha_t_e,ncol=length(s_pred),nrow=length(alpha_j_e))-as.matrix(1/smax_j_e)%*%s_pred,2,quantile,0.95))*s_pred
  
  
  RS_pred_eca_l=exp(apply(matrix(alpha_j_e+alpha_t_e,ncol=length(s_pred),nrow=length(alpha_j_e))-as.matrix(1/smax_j_e)%*%s_pred+matrix(b_ECA*min(rv$sqrt.ECA.std),ncol=length(s_pred),nrow=length(alpha_j_e)),2,median))*s_pred
  
  RS_pred_eca_u=exp(apply(matrix(alpha_j_e+alpha_t_e,ncol=length(s_pred),nrow=length(alpha_j_e))-as.matrix(1/smax_j_e)%*%s_pred+matrix(b_ECA*max(rv$sqrt.ECA.std),ncol=length(s_pred),nrow=length(alpha_j_e+alpha_t_e)),2,median))*s_pred
  ...
  #Odd
  if(nrow(rv_o)>0){
      smax_j_o=d1eca[,colnames(r1eca)== paste('S_max.',unique(rv_o$River2.n),'.',sep='')]
  alpha_t_o=apply(r1eca[,grepl('alpha_t.2',colnames(r1eca))==T],1,median)
  alpha_j_o=r1eca[,colnames(r1eca)== paste('alpha_j.',unique(rv_o$River2.n),'.',sep='')]
  b_ECA=r1eca[,colnames(r1eca)== paste('b_for_rv.',c,'.',sep='')]                           
      
    plot(Recruits~Spawners,data=rv_o,xlim=c(0,max(rv$Spawners)),ylim=c(0,max(rv$Recruits)),bty='l',type='n',main='Odd - Ricker (ECA)')
     
   s_pred=seq(0,max(rv_o$Spawners),length.out=100)
  
   RS_pred_m=exp(apply(matrix(alpha_j_e+alpha_t_e,ncol=length(s_pred),nrow=length(alpha_j_e))-as.matrix(1/smax_j_o)%*%s_pred,2,median))*s_pred
  RS_pred_l=exp(apply(matrix(alpha_j_e+alpha_t_e,ncol=length(s_pred),nrow=length(alpha_j_e))-as.matrix(1/smax_j_o)%*%s_pred,2,quantile,0.05))*s_pred
  RS_pred_u=exp(apply(matrix(alpha_j_e+alpha_t_e,ncol=length(s_pred),nrow=length(alpha_j_e))-as.matrix(1/smax_j_o)%*%s_pred,2,quantile,0.95))*s_pred
  
  
  RS_pred_eca_l=exp(apply(matrix(alpha_j_e+alpha_t_e,ncol=length(s_pred),nrow=length(alpha_j_e))-as.matrix(1/smax_j_o)%*%s_pred+matrix(b_ECA*min(rv$sqrt.ECA.std),ncol=length(s_pred),nrow=length(alpha_j_e)),2,median))*s_pred
  
  RS_pred_eca_u=exp(apply(matrix(alpha_j_e+alpha_t_e,ncol=length(s_pred),nrow=length(alpha_j_e))-as.matrix(1/smax_j_o)%*%s_pred+matrix(b_ECA*max(rv$sqrt.ECA.std),ncol=length(s_pred),nrow=length(alpha_j_e+alpha_t_e)),2,median))*s_pred
  ...
  #even
  if(nrow(rv_e)>0){
  plot(Recruits~Spawners,data=rv_e,xlim=c(0,max(rv$Spawners)),ylim=c(0,max(rv$Recruits)),bty='l',type='n',main=paste(unique(rv_e$River),'Even (CPD)',sep=' '))
  
   alpha_t_e=apply(d1cpd[,grepl('alpha_t.1',colnames(d1cpd))==T],1,median)
  alpha_j_e=d1cpd[,colnames(d1cpd)== paste('alpha_j.',unique(rv_e$River2.n),'.',sep='')]
  
  Rk_j_e=d1cpd[,colnames(d1cpd)== paste('Rk.',unique(rv_e$River2.n),'.',sep='')]
  ...
  points(Recruits~Spawners,data=rv_e,bg=col_points2,pch=24,cex=1.5) 
  }
  
  #Odd
  if(nrow(rv_o)>0){
    alpha_t_o=apply(d1cpd[,grepl('alpha_t.2',colnames(d1cpd))==T],1,median)
  alpha_j_o=d1cpd[,colnames(d1cpd)== paste('alpha_j.',unique(rv_o$River2.n),'.',sep='')]
 Rk_j_o=d1cpd[,colnames(d1cpd)== paste('Rk.',unique(rv_o$River2.n),'.',sep='')]
  
   s_pred=seq(0,max(rv_o$Spawners),length.out=100)
   
   plot(Recruits~Spawners,data=rv_o,xlim=c(0,max(rv$Spawners)),ylim=c(0,max(rv$Recruits)),bty='l',type='n',main=paste(unique(rv_o$River),'Even (CPD)',sep=' '))
    RS_pred_m=exp(apply(matrix(alpha_j_o+alpha_t_o,ncol=length(s_pred),nrow=length(alpha_j_o))-log(1+as.matrix(exp(alpha_j_o+alpha_t_o)/Rk_j_o)%*%s_pred),2,median))*s_pred
  RS_pred_l=exp(apply(matrix(alpha_j_o+alpha_t_o,ncol=length(s_pred),nrow=length(alpha_j_o))-log(1+as.matrix(exp(alpha_j_o+alpha_t_o)/Rk_j_o)%*%s_pred),2,quantile,0.05))*s_pred
  RS_pred_u=exp(apply(matrix(alpha_j_o+alpha_t_o,ncol=length(s_pred),nrow=length(alpha_j_o))-log(1+as.matrix(exp(alpha_j_o+alpha_t_o)/Rk_j_o)%*%s_pred),2,quantile,0.95))*s_pred
  ...
  points(Recruits~Spawners,data=rv_o,bg=col_points2,pch=24,cex=1.5) 
  }
 ...
 if(nrow(rv_e)>0){ 
  ...
  RS_pred_eca_l=exp(apply(matrix(alpha_j_e+alpha_t_e,ncol=length(s_pred),nrow=length(alpha_j_e))-as.matrix(1/smax_j_e)%*%s_pred+matrix(b_ECA*min(rv$sqrt.CPD.std),ncol=length(s_pred),nrow=length(alpha_j_e)),2,median))*s_pred
  
  RS_pred_eca_u=exp(apply(matrix(alpha_j_e+alpha_t_e,ncol=length(s_pred),nrow=length(alpha_j_e))-as.matrix(1/smax_j_e)%*%s_pred+matrix(b_ECA*max(rv$sqrt.CPD.std),ncol=length(s_pred),nrow=length(alpha_j_e+alpha_t_e)),2,median))*s_pred
  ...
  RS_pred_eca_l=exp(apply(matrix(alpha_j_e+alpha_t_e,ncol=length(s_pred),nrow=length(alpha_j_e))-as.matrix(1/smax_j_o)%*%s_pred+matrix(b_ECA*min(rv$sqrt.CPD.std),ncol=length(s_pred),nrow=length(alpha_j_e)),2,median))*s_pred
  
  RS_pred_eca_u=exp(apply(matrix(alpha_j_e+alpha_t_e,ncol=length(s_pred),nrow=length(alpha_j_e))-as.matrix(1/smax_j_o)%*%s_pred+matrix(b_ECA*max(rv$sqrt.CPD.std),ncol=length(s_pred),nrow=length(alpha_j_e+alpha_t_e)),2,median))*s_pred
 ...
 realized_cpd_m_r=apply(realized_cpd_r,2,median)
   ...
   plot(realized_eca_m~rv$BroodYear,bty='l',type='n',ylim=c(min(c(realized_eca_m,realized_cpd_m,realized_cpd_r,realized_eca_r)),max(c(realized_eca_m,realized_cpd_m,realized_cpd_r,realized_eca_r))*1.2),ylab='Marginal effect on productivity',xlab='Brood Year')
   lines(realized_eca_m~BroodYear,data=rv,col='navy',lwd=2)
    points(realized_eca_m~BroodYear,data=rv,col='navy',bg=col_points1,pch=21,cex=1.5) 
