...
#Chum model fits
f1ric_chm_eca=readRDS(here('stan models','outs','fits','fit1ric_chm_eca.RDS'))
f1ric_chm_cpd=readRDS(here('stan models','outs','fits','fit1ric_chm_cpd.RDS'))

f2ric_chm_eca=readRDS(here('stan models','outs','fits','fit2ric_chm_eca.RDS'))
f2ric_chm_cpd=readRDS(here('stan models','outs','fits','fit2ric_chm_cpd.RDS'))
...
This is the distribution of watershed area among 333 rivers with Pink surveys:
...
```{r, chm eca range}
plot(rep(1,nrow(da))~seq(1,333),type='n',xaxt='n',ylim=c(0,1),ylab='Range of ECA observations in River-level time-series',bty='l',xlab='')
for(i in 1:nrow(da)){
...
cu=distinct(ch20r,CU,.keep_all = T)
plot(rep(1,nrow(cu))~seq(1,22),type='n',xaxt='n',ylim=c(0,1),ylab='Range of CPD observations in CU-level time-series',bty='l',xlab='')
for(i in 1:nrow(cu)){
 s=subset(ch20r,CU==cu$CU[i])
 lines(c(min(s$disturbedarea_prct_cs),max(s$disturbedarea_prct_cs))~rep(i,2))
}
...
plot(rep(1,nrow(da))~log10(da$area_km2),type='n',xaxt='n',ylim=c(0,1),ylab='Range of CPD observations in River-level time-series',bty='l',xlab='Watershed area (km2)',xlim=c(0,log10(1e4)))
for(i in 1:nrow(da)){
 s=subset(ch20r,River==da$River[i])
 lines(c(min(s$disturbedarea_prct_cs),max(s$disturbedarea_prct_cs))~rep(unique(log10(s$area_km2)),2))
}
...
The Beverton-Holt model formulation had the highest support for chum
...
plot(m_pred~eca_n,bty='l',type='l',ylab='change in log(R/S)',xlab='Equivalent clearcut area',ylim=c(-2,2),col='darkred',lwd=3)
...
pcd_n=plogis(x_n*sd(ch20r$logit.pdisturb)+mean(ch20r$logit.pdisturb))*100
...
plot(m_pred~pcd_n,bty='l',type='l',ylab='change in log(R/S)',xlab='Cumulative % disturbed area',ylim=c(-2,2),col='darkred',lwd=3)
lines(l_pred~pcd_n,lty=5)
...
d2_eca=f2bh_chm_eca$draws(variables=c('b_ECA','b_ECA_cu','b_ECA_j'),format='draws_matrix')
d2_cpd=f2bh_chm_cpd$draws(variables=c('b_ECA','b_ECA_cu','b_ECA_j'),format='draws_matrix')

par(mfrow=c(2,2))
plot(c(0,10)~c(-2,2),bty='l',type='n',ylab='Posterior density',main='ECA effect sizes by CU',ylim=c(0,10),yaxt='n',xlab='Standardized coefficients')
d0eca=density(d2_eca[,1],bw=0.02)
for(j in 2:23){ #22 cus
d=density(d2d2_eca[,j],bw=0.02)
lines(d$y~d$x,col=adjustcolor('black',alpha.f=0.2))
}
lines(d0eca$y~d0$x,col='darkred',lwd=3)
abline(v=0,lwd=2)

plot(c(0,10)~c(-2,2),bty='l',type='n',ylab='Posterior density',main='CPD effect sizes by CU',ylim=c(0,10),yaxt='n',xlab='Standardized coefficients')
d0ecpd=density(d2_cpd[,1],bw=0.02)
for(j in 2:23){ #22 cus
...
#### Fit 2: River-level varying effects
...
#### Fit 3: Watershed area as a mediator
...
Here is the model with an alternative to ECA: the cumulative area disturbed (expressed as a % of total watershed area). This essentially relaxes the assumption of hydrological recovery

```{r, cumulative p disturb - chum fitset 1}
#create logit transformed and standardized ECA:
#predict over standardized values (x_n) and back calculate natural scale ECA (eca_n)
x_n=seq(min(ch20r$logit.pdisturb.std),max(ch20r$logit.pdisturb.std),length.out=100)
pcd_n=plogis(x_n*sd(ch20r$logit.pdisturb)+mean(ch20r$logit.pdisturb))*100

pRS=fit1bh_pd$draws(variable='b_ECA',format='draws_matrix')%*%x_n
m_pred=apply(pRS,2,median)
l_pred=apply(pRS,2,quantile,0.05)
u_pred=apply(pRS,2,quantile,0.95)

plot(m_pred~pcd_n,bty='l',type='l',ylab='change in log(R/S)',xlab='Cumulative % disturbed area')
lines(l_pred~pcd_n,lty=5)
lines(u_pred~pcd_n,lty=5)
