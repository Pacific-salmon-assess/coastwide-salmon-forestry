...
Model of log(Recruits) with ocean covariates, forestry:

$log(R_{t,r}) = log(S_{t,r}) +\alpha_r - \frac{S_{t,r}}{Smax_r} + \beta_{forestry,r}Forestry_{t,r} + \beta_{npgo,r}NPGO_{t} + \beta_{sst,r}SST_{t,r} + \rho \epsilon_{t-1,r}$
...
#need to rerun with new dataset
bh_chm_eca_st = read.csv(here('stan models','outs','posterior',
                           'bh_chm_eca_indp_alpha.csv'),check.names=F)
bh_chm_cpd_st = read.csv(here('stan models','outs','posterior',
...
```{r eval=FALSE, fig.height=8, include=FALSE, results="asis", spawner-recruit, fig.width=10}
...
### Correlation of effects of covariates between BH and Ricker models
...
cor_plot_bw_models <- function(posterior1 = ric_chm_cpd_ersst, posterior2 = bh_chm_cpd_ersst, effect = "cpd"){
  ...
  mutate(forestry_level = case_when(forestry_max < 12 ~ 'low',
    ...
    max_df <- ch20rsc %>% 
      select(River, River_n,CU, CU_name, spring_ersst, area_km2) %>% 
      group_by(River) %>%
      summarize(River_n = first(River_n),
                River = first(River),
                CU = first(CU),
                CU_name = first(CU_name),
                area_km2 = mean(area_km2, na.rm = TRUE),
                sst_max = max(spring_ersst, na.rm = TRUE))
    
    scale_color <- scale_color_gradient2(name = "Max SST level\nin River (C)",
                                         low = '#8CA8B3', mid = 'gray', high = "#C88485", midpoint = 12)
    
    scale_fill <- scale_fill_gradient2(name = "Max SST level\nin River (C)",
                                       low = '#8CA8B3', mid = 'gray', high = "#C88485", midpoint = 12)
      
      max_cu_df <- max_df %>%
        group_by(CU, CU_name) %>% 
        summarize(sst_mean = mean(sst_max, na.rm = TRUE),
                  area_km2_mean = mean(area_km2, na.rm = TRUE))
    ...
    max_df <- ch20rsc %>% 
      select(River, River_n,CU, CU_name, npgo.std, area_km2) %>% 
      group_by(River) %>%
      summarize(River_n = first(River_n),
                River = first(River),
                CU = first(CU),
                CU_name = first(CU_name),
                area_km2 = mean(area_km2, na.rm = TRUE),
                npgo_max = max(npgo.std, na.rm = TRUE))
    
    scale_color <- scale_color_gradient2(name = "Max NPGO level\nin River",
                                         low = '#8CA8B3', mid = 'gray', high = "#8CB390", midpoint = 2)
    
    scale_fill <- scale_fill_gradient2(name = "Max NPGO level\nin River",
                                       low = '#8CA8B3', mid = 'gray', high = "#8CB390", midpoint = 2)
    
    max_cu_df <- max_df %>%
      group_by(CU, CU_name) %>% 
      summarize(npgo_mean = mean(npgo_max, na.rm = TRUE),
                area_km2_mean = mean(area_km2, na.rm = TRUE))
    
    names_prefix = "b_npgo_rv"
    
  } else {
    stop("Effect must be one of 'cpd', 'eca', 'sst', or 'npgo'")
  ...
  posterior_df1 <- posterior1 %>%
    select(starts_with(names_prefix)) %>%
    pivot_longer(cols = everything(), 
                 names_to = 'River', 
  ...
  if(effect == "cpd"){
    ...
    sst_plot <- ggplot(posterior_df,aes(x = ric_effect_median,
                   y = bh_effect_median, color = sst_max)) +
    geom_point(aes(size = area_km2), alpha = 0.5) +
    annotate("text",x = 0.7*min(posterior_df$ric_effect_median), 
             y = 0.7*max(posterior_df$bh_effect_median), label = paste("Correlation = ", round(cor,2)), 
             size = 4) +
    scale_color +
    # geom_mark_ellipse(data = posterior_df %>% filter(CU == "CM-4"), aes(label = CU_name), color = "gray",
    #                   label.buffer = unit(-5, 'mm'), label.fontsize = 8) +
    ...
    npgo_plot <- ggplot(posterior_df,aes(x = ric_effect_median,
                   y = bh_effect_median, color = npgo_max)) +
    geom_point(aes(size = area_km2), alpha = 0.5) +
    annotate("text",x = 0.1*min(posterior_df$ric_effect_median), 
             y = 0.8*max(posterior_df$bh_effect_median), label = paste("Correlation = ", round(cor,2)), 
             size = 4) +
    scale_color +
    # geom_mark_ellipse(data = posterior_df %>% filter(CU == "CM-4"), aes(label = CU_name), color = "gray",
    #                   label.buffer = unit(-5, 'mm'), label.fontsize = 8) +
...
cpd_plot1 <- cor_plot_bw_models(posterior1 = ric_chm_cpd_ocean_covariates_logR, posterior2 = bh_chm_cpd_ocean_covariates, effect = "cpd")



eca_plot1 <- cor_plot_bw_models(posterior1 = ric_chm_eca_ocean_covariates_logR, posterior2 = bh_chm_eca_ocean_covariates, effect = "eca")

sst_plot1 <- cor_plot_bw_models(posterior1 = ric_chm_cpd_ocean_covariates_logR, posterior2 = bh_chm_cpd_ocean_covariates, effect = "sst")

npgo_plot1 <- cor_plot_bw_models(posterior1 = ric_chm_cpd_ocean_covariates_logR, posterior2 = bh_chm_cpd_ocean_covariates, effect = "npgo")


(cpd_plot1 + eca_plot1) / (sst_plot1 + npgo_plot1) + plot_layout(guides = "collect")
...
$log(R_{t,r}/S_{t,r}) = \alpha_r - \frac{S_{t,r}}{Smax_r} + \beta_{forestry,r}Forestry_{t,r} + \epsilon_{t,r}$
...
$log(R_{t,r}/S_{t,r}) = \alpha_r - \frac{S_{t,r}}{Smax_r} + \beta_{forestry,r}Forestry_{t,r} + \beta_{npgo,r}NPGO_{t} + \beta_{sst,r}SST_{t,r} +\epsilon_{t,r}$
...
ric_pk_eca_ocean_covariates = read.csv(here('stan models',
                        'outs',
                        'posterior',
                        'ric_pk_eca_st_noac_ocean_covariates_logR.csv'),
                        check.names=F)




ric_pk_cpd_ocean_covariates = read.csv(here('stan models',
                        'outs',
                        'posterior',
                        'ric_pk_cpd_st_noac_ocean_covariates_logR.csv'),
                        check.names=F)
...
(plot_forestry_effect(posterior = ric_pk_cpd_st, effect = "cpd", species = "pink", model = "Ricker model with CPD", xlim = c(-0.5,0.5)) +
    plot_forestry_effect(posterior = ric_pk_eca_st, effect = "eca", species = "pink", model = "Ricker model with ECA", xlim = c(-0.5,0.5)))/(plot_forestry_effect(posterior = ric_pk_cpd, effect = "cpd", species = "pink", model = "Ricker model with CPD,\ncoastwide productivity", xlim = c(-0.5,0.5)) +
    plot_forestry_effect(posterior = ric_pk_eca, effect = "eca", species = "pink", model = "Ricker model with ECA,\ncoastwide productivity", xlim = c(-0.5,0.5)))/(plot_forestry_effect(posterior = ric_pk_cpd_ocean_covariates, effect = "cpd", species = "pink", model = "Ricker model with CPD, NPGO, SST", xlim = c(-0.5,0.5)) +
    plot_forestry_effect(posterior = ric_pk_eca_ocean_covariates,
                         effect = "eca", species = "pink", model = "Ricker model with ECA, NPGO, SST", xlim = c(-0.5,0.5)))+ plot_layout(guides = "collect")
...
$log(R_{t,r}/S_{t,r}) = \alpha_r - log(1+(\frac{e^{\alpha_r + \beta_{forestry,r}Forestry_{t,r} }}{R_{k,r}})S_{t,r}) + \beta_{forestry,r}Forestry_{t,r} + \epsilon_{t,r}$
...
$log(R_{t,r}/S_{t,r}) = \alpha_r - log(1+(\frac{e^{\alpha_t+\alpha_r + \beta_{forestry,r}Forestry_{t,r}+ \beta_{npgo,r}NPGO_{t} + \beta_{sst,r}SST_{t,r} }}{R_{k,r}})S_{t,r}) + \beta_{forestry,r}Forestry_{t,r} + \beta_{npgo,r}NPGO_{t} + \beta_{sst,r}SST_{t,r} + \epsilon_{t,r}$
...
(plot_forestry_effect(posterior = bh_pk_cpd_st, effect = "cpd", species = "pink", model = "BH model with CPD", xlim = c(-0.5,0.5)) + plot_forestry_effect(posterior = bh_pk_eca_st, effect = "eca", species = "pink", model = "BH model with ECA", xlim = c(-0.5,0.5)))/(plot_forestry_effect(posterior = bh_pk_cpd, effect = "cpd", species = "pink", model = "BH model with CPD,\ncoastwide productivity", xlim = c(-0.5,0.5)) + plot_forestry_effect(posterior = bh_pk_eca, effect = "eca", species = "pink", model = "BH model with ECA,\ncoastwide productivity", xlim = c(-0.5,0.5)))/(plot_forestry_effect(posterior = bh_pk_cpd_ersst, effect = "cpd", species = "pink", model = "BH model with NPGO, SST", xlim = c(-0.5,0.5)) + plot_forestry_effect(posterior = bh_pk_eca_ersst, effect = "eca", species = "pink", model = "BH model with NPGO, SST", xlim = c(-0.5,0.5))) + plot_layout(guides = "collect")
...
```{r, residual-plots-pink, fig.width = 12, fig.height = 8, results="asis"}
  
df_pk <- pk10r

posterior_pk <- ric_pk_cpd_ersst
# posterior_pk <- ric_pk_cpd_ocean_covariates

posterior_pk_bh <- bh_pk_cpd_ersst



rivers <- unique(df_pk$River_n2)

# plotting residuals of ricker model vs predicted values of ricker model for all rivers combined

residual_df_full_pink <- data.frame()

residual_bh_df_full_pink <- data.frame()

for(river2 in rivers){
  
  river_data <- df_pk %>% filter(River_n2 == river2)
  
  river <- river_data$River_n
  
  posterior_rv_b_for <- posterior_pk %>% 
      select(starts_with('b_for_rv')) %>%
      select(ends_with(paste0("[",river,"]")))
  
  posterior_rv_b_npgo <- posterior_pk %>% 
      select(starts_with('b_npgo_rv')) %>%
      select(ends_with(paste0("[",river,"]")))
  
  posterior_rv_b_sst <- posterior_pk %>% 
      select(starts_with('b_sst_rv')) %>%
      select(ends_with(paste0("[",river,"]")))
  
  posterior_rv_alpha_j <- posterior_pk %>% 
      select(starts_with('alpha_j')) %>%
      select(ends_with(paste0("[",river2,"]")))
  
  posterior_rv_S_max <- posterior_pk %>% 
      select(starts_with('Smax')) %>%
      select(ends_with(paste0("[",river2,"]")))
  ...
  residual_df <- data.frame(spawners = river_data$Spawners,
    ...
    residual_df_full_pink <- rbind(residual_df_full_pink, residual_df)
    
    posterior_bh_rv_b_for <- posterior_pk_bh %>% 
      select(starts_with('b_for_rv')) %>%
      select(ends_with(paste0("[",river,"]")))
    
    posterior_bh_rv_b_npgo <- posterior_pk_bh %>% 
      select(starts_with('b_npgo_rv')) %>%
      select(ends_with(paste0("[",river,"]")))
    
    posterior_bh_rv_b_sst <- posterior_pk_bh %>%
      select(starts_with('b_sst_rv')) %>%
      select(ends_with(paste0("[",river,"]")))
    
    posterior_bh_rv_alpha_j <- posterior_pk_bh %>%
      select(starts_with('alpha_j')) %>%
      select(ends_with(paste0("[",river2,"]")))
    
    posterior_bh_rv_Rk <- posterior_pk_bh %>%
      select(starts_with('Rk')) %>%
      select(ends_with(paste0("[",river2,"]")))
    ...
    residual_bh_df_full_pink <- rbind(residual_bh_df_full_pink, residual_bh_df)
    
}

p_ric_residuals_pink <- ggplot(residual_df_full_pink)+
  geom_point(aes(x = predicted_log_RS, y = residuals, color = cpd), alpha = 0.2, size = 2) +
  geom_hline(yintercept = 0, color = 'black', linetype = 'dashed') +
  labs(title = "Ricker model with CPD, NPGO, ERSST", x = TeX(r"(Predicted $\log \left(\frac{Recruits}{Spawners}\right)$)"), y = "Residuals") +
  scale_color_gradient2(name = 'CPD (%)',
...
p_ric_partial_residuals_pink <- ggplot(residual_df_full_pink)+
  geom_point(aes(x = sqrt.CPD.std, y = partial_residuals, color = cpd), alpha = 0.2, size = 2) +
  geom_hline(yintercept = 0, color = 'black', linetype = 'dashed') +
  geom_smooth(aes(x = sqrt.CPD.std, y = partial_residuals), method = 'lm', se = TRUE, color = 'black') +
  labs(title = "Ricker model with CPD, NPGO, ERSST", x = TeX(r"($standardized\ \sqrt{CPD}$)"), 
...
p_bh_residuals_pink <- ggplot(residual_bh_df_full_pink)+
  geom_point(aes(x = predicted_log_RS, y = residuals, color = cpd), alpha = 0.2, size = 2) +
  geom_hline(yintercept = 0, color = 'black', linetype = 'dashed') +
  labs(title = "BH model with CPD, NPGO, ERSST", x = TeX(r"(Predicted $\log \left(\frac{Recruits}{Spawners}\right)$)"), y = "Residuals") +
  scale_color_gradient2(name = 'CPD (%)',
...
p_bh_partial_residuals_pink <- ggplot(residual_bh_df_full_pink)+
  geom_point(aes(x = sqrt.CPD.std, y = partial_residuals, color = cpd), alpha = 0.2, size = 2) +
  geom_hline(yintercept = 0, color = 'black', linetype = 'dashed') +
  geom_smooth(aes(x = sqrt.CPD.std, y = partial_residuals), method = 'lm', se = TRUE, color = 'black') +
  labs(title = "BH model with CPD, NPGO, ERSST", x = TeX(r"($standardized\ \sqrt{CPD}$)"), 
...
print((p_ric_residuals_pink + p_bh_residuals_pink)/(p_ric_partial_residuals_pink + p_bh_partial_residuals_pink) + plot_layout(guides = "collect"))
    
## color by CU

p_cu_ric_residuals_pink <- ggplot(residual_df_full_pink)+
  geom_point(aes(x = predicted_log_RS, y = residuals, color = CU_name), alpha = 0.2, size = 2) +
  geom_hline(yintercept = 0, color = 'black', linetype = 'dashed') +
  labs(title = "Ricker model with CPD, NPGO, ERSST", x = TeX(r"(Predicted $\log \left(\frac{Recruits}{Spawners}\right)$)"), y = "Residuals") +
  scale_color_iwanthue(name = 'CU', hmin = 0, hmax = 360, cmin = 0, cmax = 100,
...
p_cu_partial_ric_residuals_pink <- ggplot(residual_df_full_pink)+
  geom_point(aes(x = sqrt.CPD.std, y = partial_residuals, color = CU_name), alpha = 0.2, size = 2) +
  geom_hline(yintercept = 0, color = 'black', linetype = 'dashed') +
  geom_smooth(aes(x = sqrt.CPD.std, y = partial_residuals), method = 'lm', se = TRUE, color = 'black') +
  labs(title = "Ricker model with CPD, NPGO, ERSST", x = TeX(r"($standardized\ \sqrt{CPD}$)"), 
...
p_bh_cu_residuals_pink <- ggplot(residual_bh_df_full_pink)+
  geom_point(aes(x = predicted_log_RS, y = residuals, color = CU_name), alpha = 0.2, size = 2) +
  geom_hline(yintercept = 0, color = 'black', linetype = 'dashed') +
  labs(title = "BH model with CPD, NPGO, ERSST", x = TeX(r"(Predicted $\log \left(\frac{Recruits}{Spawners}\right)$)"), y = "Residuals") +
  scale_color_iwanthue(name = 'CU', hmin = 0, hmax = 360, cmin = 0, cmax = 100,
...
p_bh_cu_partial_residuals_pink <- ggplot(residual_bh_df_full_pink)+
  geom_point(aes(x = sqrt.CPD.std, y = partial_residuals, color = CU_name), alpha = 0.2, size = 2) +
  geom_hline(yintercept = 0, color = 'black', linetype = 'dashed') +
  geom_smooth(aes(x = sqrt.CPD.std, y = partial_residuals), method = 'lm', se = TRUE, color = 'black') +
  labs(title = "BH model with CPD, NPGO, ERSST", x = TeX(r"($standardized\ \sqrt{CPD}$)"), 
...
print((p_cu_ric_residuals_pink + p_bh_cu_residuals_pink)/(p_cu_partial_ric_residuals_pink + p_bh_cu_partial_residuals_pink) + plot_layout(guides = "collect"))
...
for(river2 in rivers){
  ...
  residual_df <- data.frame(spawners = river_data$Spawners,
                              recruits = river_data$Recruits,
                              observed_log_R = log(river_data$Recruits),
                              predicted_log_R = apply(matrix(log(river_data$Spawners),ncol = length(river_data$Spawners), nrow = length(posterior_rv_alpha_j[,1]) ) +(matrix(posterior_rv_alpha_j[,1], ncol = length(river_data$Spawners), nrow = length(posterior_rv_alpha_j[,1])) - as.matrix(1/posterior_rv_S_max)%*%river_data$Spawners + as.matrix(posterior_rv_b_for)%*%river_data$sqrt.CPD.std +
                                                          as.matrix(posterior_rv_b_npgo)%*%river_data$npgo.std + 
                                                          as.matrix(posterior_rv_b_sst)%*%river_data$sst.std ), 2, median),
...
p_ric_residuals_pink <- ggplot(residual_df_full_pink)+
  geom_point(aes(x = predicted_log_R, y = residuals, color = cpd), alpha = 0.2, size = 2) +
  geom_hline(yintercept = 0, color = 'black', linetype = 'dashed') +
  labs(title = "Ricker model with CPD, NPGO, ERSST", x = TeX(r"(Predicted $\log \left(Recruits\right)$)"), y = "Residuals") +
  scale_color_gradient2(name = 'CPD (%)',
                                        low = '#35978f', mid = 'gray', high = '#bf812d', midpoint = 50)+
  # ylim(-6, 6) +
  theme_classic() +
...
p_cu_ric_residuals_pink <- ggplot(residual_df_full_pink)+
  geom_point(aes(x = predicted_log_R, y = residuals, color = CU_name), alpha = 0.2, size = 2) +
  geom_hline(yintercept = 0, color = 'black', linetype = 'dashed') +
  labs(title = "Ricker model with CPD, NPGO, ERSST", x = TeX(r"(Predicted $\log \left(Recruits\right)$)"), y = "Residuals") +
  scale_color_iwanthue(name = 'CU', hmin = 0, hmax = 360, cmin = 0, cmax = 100,
                         lmin = 10, lmax = 95)+
