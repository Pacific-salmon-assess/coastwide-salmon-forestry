...
plot_predicted_recruits <- function(posterior1 = ric_chm_cpd_ocean_covariates_logR,
  ...
  predicted_recruits <- data.frame(BroodYear = rep(river_data$BroodYear, each = nrow(posterior_df_b_for)),
                                    Spawners = rep(river_data$Spawners, each = nrow(posterior_df_b_for)),
                                    forestry = rep(river_data$sqrt.CPD.std, each = nrow(posterior_df_b_for)),
                                    alpha_j = rep(posterior_df_alpha$alpha_j, times = nrow(river_data)),
                                    Smax = rep(posterior_df_Smax$Smax, times = nrow(river_data))
                                    )
