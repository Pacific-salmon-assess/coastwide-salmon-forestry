...
((plot_sst_effect(posterior = bh_chm_eca_sst, effect = "eca", species = "chum", model = "BH model with NPGO, SST", xlim = c(-0.5,0.5)) +
  ...
  plot_sst_effect(posterior = bh_chm_cpd_cobe_sst, effect = "cpd", species = "chum", model = "BH model with NPGO, COBE SST", xlim = c(-0.5,0.5)))/(plot_sst_effect(posterior = bh_chm_eca_ersst, effect = "eca", species = "chum", model = "BH model with NPGO, ERSST", xlim = c(-0.5,0.5)) + plot_sst_effect(posterior = bh_chm_cpd_ersst, effect = "cpd", species = "chum", model = "BH model with NPGO, ERSST", xlim = c(-0.5,0.5)))+ plot_layout(guides = "collect"))
...
((plot_sst_effect(posterior = ric_chm_eca_sst, effect = "eca", species = "chum", model = "Ricker model with NPGO, SST", xlim = c(-0.5,0.5)) +
  ...
  plot_sst_effect(posterior = ric_chm_cpd_lh_sst, effect = "cpd", species = "chum", model = "Ricker model with NPGO, LH SST", xlim = c(-0.5,0.5)))/(plot_sst_effect(posterior = ric_chm_eca_cobe_sst, effect = "eca", species = "chum", model = "Ricker model with NPGO, COBE SST", xlim = c(-0.5,0.5)) + plot_sst_effect(posterior = ric_chm_cpd_cobe_sst, effect = "cpd", species = "chum", model = "Ricker model with NPGO, COBE SST", xlim = c(-0.5,0.5)))
   + plot_layout(guides = "collect")
  )
...
# Correlation between COBE SST, LH SST, ERSST
...
chum_ersst_data <- read.csv(here('origional-ecofish-data-models','Data','Processed',
...
chum_sst_data <- merge(chum_lh_data %>% select(BroodYear, River, spring_lighthouse_temperature, CU) %>% distinct(),
                       ...
                       by = c('BroodYear', 'River', 'CU')) %>% 
  left_join(cu_names, by = 'CU')

chum_sst_data <- merge(chum_sst_data,
                       chum_ersst_data %>% select(BroodYear, River, spring_ersst, CU) %>% distinct(),
                       by = c('BroodYear', 'River', 'CU'))


correlation = cor(chum_sst_data$spring_lighthouse_temperature, chum_sst_data$spring_sst)

correlation_lh_ersst = cor(chum_sst_data$spring_lighthouse_temperature, chum_sst_data$spring_ersst)

correlation_cobe_ersst = cor(chum_sst_data$spring_sst, chum_sst_data$spring_ersst)

cor_lh_cobe <- ggplot(chum_sst_data) +
...
cor_lh_ersst <- ggplot(chum_sst_data) +
  geom_point(aes(x = spring_lighthouse_temperature, y = spring_ersst, color = CU_name), size = 2, alpha = 0.1) +
  labs(x = 'Lighthouse SST', y = 'ERSST', title = 'Correlation between ERSST and LH SST') +
  scale_color_iwanthue(name = 'CU', hmin = 0, hmax = 360, cmin = 0, cmax = 80,
                         lmin = 10, lmax = 95) +
  xlim(8, 14) +
  ylim(8, 14) +
  ...
  #plot 1 to 1 line
  geom_abline(intercept = 0, slope = 1, linetype = 'dashed', color = 'black') +
  annotate("text", x = 12, y = 9, label = paste('Correlation:', round(correlation_lh_ersst, 2)), size = 5)+
  theme(legend.key.height = unit(1, 'lines'),
        legend.position = 'right',
        legend.title = element_text(size = 10),
        legend.text = element_text(size = 8),
...
cor_cobe_ersst <- ggplot(chum_sst_data) +
  geom_point(aes(x = spring_sst, y = spring_ersst, color = CU_name), size = 2, alpha = 0.1) +
  labs(x = 'COBE SST', y = 'ERSST', title = 'Correlation between ERSST and COBE SST') +
  scale_color_iwanthue(name = 'CU', hmin = 0, hmax = 360, cmin = 0, cmax = 80,
                         lmin = 10, lmax = 95) +
  xlim(8, 14) +
  ylim(8, 14) +
  ...
  #plot 1 to 1 line
  geom_abline(intercept = 0, slope = 1, linetype = 'dashed', color = 'black') +
  annotate("text", x = 12, y = 9, label = paste('Correlation:', round(correlation_cobe_ersst, 2)), size = 5)+
  theme(legend.key.height = unit(1, 'lines'),
        legend.position = 'right',
        legend.title = element_text(size = 10),
        legend.text = element_text(size = 8),
...
(cor_lh_cobe / cor_lh_ersst / cor_cobe_ersst) + plot_layout(guides = "collect")
...
# Productivity decline due to SST

```{r fig.width=5, fig.height=5}

# only use model with ersst
# 
# plot_productivity_decline(posterior = bh_chm_cpd_ersst, effect = "sst", species = "chum", model = "BH model with NPGO, ERSST")
