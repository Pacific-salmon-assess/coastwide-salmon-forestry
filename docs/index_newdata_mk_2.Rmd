...
bh_chm_eca_sst_rv <- bh_chm_eca_sst %>%
  ...
  select(-River) %>%
  left_join(max_eca_df, by = 'River_n')


#color by river
bh_chm_sst_p1 <- ggplot() +
  stat_density(data= bh_chm_eca_sst_rv, aes(eca_effect, color = eca_level, group = River),
               geom = 'line', position = 'identity', 
               alpha = 0.1, linewidth = 1) +
  geom_vline(xintercept = 0, color = 'slategray', linewidth = 0.8) +
  labs(title = 'BH model with NPGO, LH SST', 
       x = 'Standardized coefficients of ECA', y = 'Posterior density of ECA effect') +
  xlim(-2, 2) +
  scale_color_manual(name = "Max ECA level\nin River (%)",
                     values = c('low' = '#35978f', 'medium' = 'gray', 'high' = '#bf812d')) +
  geom_density(aes(bh_chm_eca_sst$b_for), color = 'black', linewidth = 1.2, alpha = 0.2)+
  theme_classic()+
...
bh_chm_cpd_sst_rv <- bh_chm_cpd_sst %>%
  select(starts_with('b_for_rv')) %>%
  pivot_longer(cols = everything(), 
               names_to = 'River', 
               names_prefix = 'b_for_rv',
  ...
  mutate(River_n = as.numeric(str_extract(River, '\\d+'))) %>% 
  select(-River) %>%
  left_join(max_cpd_df, by = 'River_n')

bh_chm_sst_p2 <- ggplot() +
  stat_density(data= bh_chm_cpd_sst_rv, aes(cpd_effect, color = cpd_max, group = River),
               geom = 'line', position = 'identity', 
               alpha = 0.1, linewidth = 1) +
  geom_vline(xintercept = 0, color = 'slategray', linewidth = 0.8) +
  labs(title = 'BH model with NPGO, LH SST', 
       x = 'Standardized coefficients of CPD', y = 'Posterior density of CPD effect') +
  xlim(-2, 2) +
  scale_color_gradient2(name = 'Max CPD\nin River (%)',
                        low = '#35978f', mid = 'gray', high = '#bf812d', midpoint = 50) +
  geom_density(aes(bh_chm_cpd_sst$b_for), color = 'black', linewidth = 1.2, alpha = 0.2)+
  theme_classic()+
  theme(legend.position = 'right',
        axis.title.x = element_text(size = 12),
        axis.title.y = element_text(size = 12),
...
bh_pk_eca_npgo_rv <- bh_pk_eca_npgo %>%
  ...
  mutate(River_n = as.numeric(str_extract(River, '\\d+'))) %>% 
  select(-River) %>%
  left_join(max_eca_pink_df, by = 'River_n')

bh_pk_npgo_p1 <- ggplot() +
  stat_density(data= bh_pk_eca_npgo_rv, aes(eca_effect, color = eca_level, group = River),
               geom = 'line', position = 'identity', 
               alpha = 0.1, linewidth = 1) +
  geom_vline(xintercept = 0, color = 'slategray', linewidth = 0.8) +
  labs(title = 'BH model with NPGO', 
       x = 'Standardized coefficients of ECA', y = 'Posterior density of ECA effect') +
  xlim(-0.5, 0.5) +
  scale_color_manual(name = "Max ECA level\nin River (%)",
                     values = c('low' = '#35978f', 'medium' = 'gray', 'high' = '#bf812d')) +
  geom_density(aes(bh_pk_eca_npgo$b_for), color = 'black', linewidth = 1.2, alpha = 0.2)+
  theme_classic()+
  theme(legend.position = 'right',
        axis.title.x = element_text(size = 12),
        axis.title.y = element_text(size = 12),
...
bh_pk_cpd_npgo_rv <- bh_pk_cpd_npgo %>%
  select(starts_with('b_for_rv')) %>%
  pivot_longer(cols = everything(), 
               names_to = 'River', 
               names_prefix = 'b_for_rv',
  ...
  mutate(River_n = as.numeric(str_extract(River, '\\d+'))) %>% 
  select(-River) %>%
  left_join(max_cpd_pink_df, by = 'River_n')

bh_pk_npgo_p2 <- ggplot() +
  stat_density(data= bh_pk_cpd_npgo_rv, aes(cpd_effect, color = cpd_max, group = River),
               geom = 'line', position = 'identity', 
               alpha = 0.1, linewidth = 1) +
  geom_vline(xintercept = 0, color = 'slategray', linewidth = 0.8) +
  labs(title = 'BH model with NPGO', 
  ...
  scale_color_gradient2(name = 'Max CPD\nin River (%)',
                        low = '#35978f', mid = 'gray', high = '#bf812d', midpoint = 50) +
  geom_density(aes(bh_pk_cpd_npgo$b_for), color = 'black', linewidth = 1.2, alpha = 0.2)+
  theme_classic()+
  theme(legend.position = 'right',
        axis.title.x = element_text(size = 12),
        axis.title.y = element_text(size = 12),
...
bh_pk_eca_sst_rv <- bh_pk_eca_sst %>%
  select(starts_with('b_for_rv')) %>%
  pivot_longer(cols = everything(), 
               names_to = 'River', 
               names_prefix = 'b_for_rv',
...
#color by river
bh_pk_sst_p1 <- ggplot() +
  stat_density(data= bh_pk_eca_sst_rv, aes(eca_effect, color = eca_level, group = River),
               geom = 'line', position = 'identity', 
               alpha = 0.1, linewidth = 1) +
  geom_vline(xintercept = 0, color = 'slategray', linewidth = 0.8) +
  labs(title = 'BH model with NPGO, LH SST', 
  ...
  scale_color_manual(name = "Max ECA level\nin River (%)",
                     values = c('low' = '#35978f', 'medium' = 'gray', 'high' = '#bf812d')) +
  geom_density(aes(bh_pk_eca_sst$b_for), color = 'black', linewidth = 1.2, alpha = 0.2)+
  theme_classic()+
  theme(legend.position = 'right',
        axis.title.x = element_text(size = 12),
        axis.title.y = element_text(size = 12),
...
bh_pk_cpd_sst_rv <- bh_pk_cpd_sst %>%
  select(starts_with('b_for_rv')) %>%
  pivot_longer(cols = everything(), 
               names_to = 'River', 
               names_prefix = 'b_for_rv',
...
bh_pk_sst_p2 <- ggplot() +
  stat_density(data= bh_pk_cpd_sst_rv, aes(cpd_effect, color = cpd_max, group = River),
               geom = 'line', position = 'identity', 
               alpha = 0.1, linewidth = 1) +
  geom_vline(xintercept = 0, color = 'slategray', linewidth = 0.8) +
  labs(title = 'BH model with NPGO, LH SST', 
  ...
  scale_color_gradient2(name = 'Max CPD\nin River (%)',
                        low = '#35978f', mid = 'gray', high = '#bf812d', midpoint = 50) +
  geom_density(aes(bh_pk_cpd_sst$b_for), color = 'black', linewidth = 1.2, alpha = 0.2)+
  theme_classic()+
  theme(legend.position = 'right',
        axis.title.x = element_text(size = 12),
        axis.title.y = element_text(size = 12),
...
(bh_pk_npgo_p1 + bh_pk_npgo_p2)/(bh_pk_sst_p1 + bh_pk_sst_p2)


```

### Effect of NPGO and SST

```{r}

bh_pk_npgo_rv <- bh_pk_eca_npgo %>%
  select(starts_with('b_npgo_rv')) %>%
  pivot_longer(cols = everything(), 
               names_to = 'River', 
               names_prefix = 'b_npgp_rv',
