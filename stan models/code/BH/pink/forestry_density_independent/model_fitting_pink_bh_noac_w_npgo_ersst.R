...
if(Sys.info()[7] == "mariakur") {
  ...
  bh_pk_eca <- mbh_p$sample(data=dl_pk_eca,
                            ...
                            adapt_delta = 0.999,
                            max_treedepth = 20)
  write.csv(bh_pk_eca$summary(),'./stan models/outs/summary/bh_pk_eca_noac_w_npgo_ersst_trial.csv')
  bh_pk_eca$save_object('./stan models/outs/fits/bh_pk_eca_noac_w_npgo_ersst_trial.RDS')
  ...
  write.csv(post_bh_pk_eca,here('stan models','outs','posterior','bh_pk_eca_noac_w_npgo_ersst_trial.csv'))
  ...
  tryCatch(
        ...
        {
        #some expression
          write.csv(bh_pk_eca$summary(),here("stan models","outs","summary","bh_pk_eca_noac_w_npgo_ersst.csv"))
          bh_pk_eca$save_object(here("stan models","outs","fits","bh_pk_eca_noac_w_npgo_ersst.RDS"))
  
          post_bh_pk_eca=bh_pk_eca$draws(variables=c('b_for','b_for_cu','b_for_rv',
                                                     'b_npgo','b_npgo_cu','b_npgo_rv',
                                                     'b_sst','b_sst_cu','b_sst_rv',
                                                     'alpha_j','Rk','sigma_for_cu','sigma_for_rv'),format='draws_matrix')
          write.csv(post_bh_pk_eca,here('stan models','outs','posterior','bh_pk_eca_noac_w_npgo_ersst.csv'))
        },
...
if(Sys.info()[7] == "mariakur") {
  ...
  write.csv(bh_pk_cpd$summary(),'./stan models/outs/summary/bh_pk_cpd_noac_w_npgo_ersst_trial.csv')
  bh_pk_cpd$save_object('./stan models/outs/fits/bh_pk_cpd_noac_w_npgo_ersst_trial.RDS')
  
  post_bh_pk_cpd=bh_pk_cpd$draws(variables=c('b_for','b_for_cu','b_for_rv',
                                             'b_npgo','b_npgo_cu','b_npgo_rv',
                                             'b_sst','b_sst_cu','b_sst_rv',
                                             'alpha_j','Rk','sigma_for_cu','sigma_for_rv'),format='draws_matrix')
  write.csv(post_bh_pk_cpd,here('stan models','outs','posterior','bh_pk_cpd_noac_w_npgo_ersst_trial.csv'))
  ...
  tryCatch(
        ...
        {
        #some expression
          write.csv(bh_pk_cpd$summary(),here("stan models","outs","summary","bh_pk_cpd_noac_w_npgo_ersst.csv"))
          bh_pk_cpd$save_object(here("stan models","outs","fits","bh_pk_cpd_noac_w_npgo_ersst.RDS"))
  
          post_bh_pk_cpd=bh_pk_cpd$draws(variables=c('b_for','b_for_cu','b_for_rv',
                                                     'b_npgo','b_npgo_cu','b_npgo_rv',
                                                     'b_sst','b_sst_cu','b_sst_rv',
                                                     'alpha_j','Rk','sigma_for_cu','sigma_for_rv'),format='draws_matrix')
          write.csv(post_bh_pk_cpd,here('stan models','outs','posterior','bh_pk_cpd_noac_w_npgo_ersst.csv'))
        },
        #if an error occurs, tell me the error
        error=function(e) {
            message('An Error Occurred')
