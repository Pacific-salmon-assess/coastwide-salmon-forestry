...
vector[N] alpha_j_for; //productivity estimates with forestry effects for each stock
...
//residual productivity deviations
   ...
   alpha_j_for[start_y[j]]=alpha_j[j]+b_for_rv[R_i[j]]*forest_loss[start_y[j]]; //initial alpha with forestry effect
   ...
   e_t[start_y[j]] = R_S[start_y[j]] - mu1[start_y[j]]; //first deviate for stock j
	...
	for(t in (start_y[j]+1):(end_y[j])){ //adjust expectation based on autocorrelation
	  alpha_j_for[t]=alpha_j[j]+b_for_rv[R_i[j]]*forest_loss[t]; //alpha with forestry effect
