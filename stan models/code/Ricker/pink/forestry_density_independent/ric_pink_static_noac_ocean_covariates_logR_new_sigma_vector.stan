...
parameters{
  ...
  vector<lower=0>[C] sigma_a_j; //variance in long-term productivity among stocks #changing to vector to match chum models
...
transformed parameters{	
  ...
  alpha_j = alpha0[BL]+alpha_cu[C_i]+ sigma_a_j[C_i].*z_a_j; //estimate for River time-invariant productivity
  ...
  e_t[t] = logR[t] - mu2[t];   //residual for stock j at time t
...
generated quantities{
  ...
  for(j in 1:J){
    ...
    for(t in (start_y[j]+1):(end_y[j])){
      log_lik[t]=normal_lpdf(logR[t]|mu2[t], sigma[j]); //pointwise log likelihood calculation
    }
  }
}
