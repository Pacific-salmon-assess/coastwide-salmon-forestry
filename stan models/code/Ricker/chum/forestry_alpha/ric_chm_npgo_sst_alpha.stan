...
transformed parameters{
  ...
  vector[N] alpha_for ; //time varying alpha with forestry, npgo, sst
  vector<lower=0>[J] b; 
  ...
  for(j in 1:J){//for every stock
    alpha_for[start_y[j]] = alpha_j[j] + b_for_rv[j]*forest_loss[start_y[j]] + b_npgo_rv[j]*npgo[start_y[j]] + b_sst_rv[j]*sst[start_y[j]]; //adjust productivity based on forest loss, npgo, sst
    b[j] = 1/K[j];
    mu1[start_y[j]]=alpha_for[start_y[j]]*(1-b[j]*S[start_y[j]]); 
    ...
    for(t in (start_y[j]+1):(end_y[j])){ //adjust expectation based on autocorrelation
      alpha_for[t] = alpha_j[j] + b_for_rv[j]*forest_loss[t] + b_npgo_rv[j]*npgo[t] + b_sst_rv[j]*sst[t]; //adjust productivity based on forest loss
      mu2[t]  = alpha_for[t]*(1 - b[j]*S[t]) + rho[j]^(ii[t]-ii[t-1])*e_t[t-1]; //adjust expectation based on previous deviate - rho is raised to the power of the number of time steps (in years) between observations
