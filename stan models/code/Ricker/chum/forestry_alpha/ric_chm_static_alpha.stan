...
transformed parameters{
  ...
  vector[N] alpha_for ; //time varying alpha
  vector<lower=0>[J] b; //time varying b
  ...
  for(j in 1:J){//for every stock
    alpha_for[start_y[j]] = alpha_j[j] + b_for_rv[j]*forest_loss[start_y[j]]; //adjust productivity based on forest loss
    b[j] = 1/K[j];
    ...
    for(t in (start_y[j]+1):(end_y[j])){ //adjust expectation based on autocorrelation
      alpha_for[t] = alpha_j[j] + b_for_rv[j]*forest_loss[t]; //adjust productivity based on forest loss
