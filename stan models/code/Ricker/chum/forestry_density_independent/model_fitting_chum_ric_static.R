...
file_ric=file.path(here('stan models', 'code', 'ric_chm_static.stan'))
...
#standardize npgo
# ch20r$npgo.std=(ch20r$npgo_new-mean(ch20r$npgo_new))/sd(ch20r$npgo_new)
...
#data list for fits
dl_chm_eca_static=list(N=nrow(ch20r),
...
dl_chm_cpd_static=list(N=nrow(ch20r),
                     L=max(ch20r$BroodYear)-min(ch20r$BroodYear)+1,
                     C=length(unique(ch20r$CU)),
                     J=length(unique(ch20r$River)),
                     C_i=as.numeric(factor(cu$CU)), #CU index by stock
...
if(Sys.info()[7] == "mariakur") {
  print("Running on local machine")
  ric_chm_eca_static <- mric$sample(data=dl_chm_eca_staic,
                            chains = 1, 
                            iter_warmup = 20,
                            iter_sampling =50,
                            refresh = 10,
                            adapt_delta = 0.999,
                            max_treedepth = 20)
  write.csv(ric_chm_eca_static$summary(),'./stan models/outs/summary/ric_chm_eca_static_trial_nov24.csv')
  ric_chm_eca_static$save_object('./stan models/outs/fits/ric_chm_eca_static_trial_nov24.RDS')
  
  post_ric_chm_eca_static=ric_chm_eca_static$draws(variables=c('b_for','b_for_cu','b_for_rv',
                                                           # 'b_npgo','b_npgo_cu','b_npgo_rv',
                                                           'alpha_j','Smax','sigma'),format='draws_matrix')
  write.csv(post_ric_chm_eca_static,here('stan models','outs','posterior','ric_chm_eca_static_trial_nov24.csv'))
  
} else {
  ric_chm_eca_static <- mric$sample(data=dl_chm_eca_static,
                            chains = 6, 
                            iter_warmup = 200,
                            iter_sampling =500,
                            refresh = 100,
  ...
  write.csv(ric_chm_eca_static$summary(),here("stan models","outs","summary","ric_chm_eca_static_nov24.csv"))
  ric_chm_eca_static$save_object(here("stan models","outs","fits","ric_chm_eca_static_nov24.RDS"))
  
  post_ric_chm_eca_static=ric_chm_eca_static$draws(variables=c('b_for','b_for_cu','b_for_rv',
                                                           # 'b_npgo','b_npgo_cu','b_npgo_rv',
                                                           'alpha_j','Smax','sigma'),format='draws_matrix')
  write.csv(post_ric_chm_eca_static,here('stan models','outs','posterior','ric_chm_eca_static_nov24.csv'))
...
if(Sys.info()[7] == "mariakur") {
  print("Running on local machine")
  ric_chm_cpd_static <- mric$sample(data=dl_chm_cpd_static,
                                  chains = 1, 
                                  iter_warmup = 20,
                                  iter_sampling =50,
                                  refresh = 10,
                                  adapt_delta = 0.999,
                                  max_treedepth = 20)
  write.csv(ric_chm_cpd_static$summary(),'./stan models/outs/summary/ric_chm_cpd_static_trial_nov24.csv')
  ric_chm_cpd_static$save_object('./stan models/outs/fits/ric_chm_cpd_static_trial_nov24.RDS')
  
  post_ric_chm_cpd_static=ric_chm_cpd_static$draws(variables=c('b_for','b_for_cu','b_for_rv',
                                                           # 'b_npgo','b_npgo_cu','b_npgo_rv',
                                                           'alpha_j','Smax','sigma'),format='draws_matrix')
  write.csv(post_ric_chm_cpd_static,here('stan models','outs','posterior','ric_chm_cpd_static_trial_nov24.csv'))
  ...
  write.csv(ric_chm_cpd_static$summary(),here("stan models","outs","summary","ric_chm_cpd_static_nov24.csv"))
  ric_chm_cpd_static$save_object(here("stan models","outs","fits","ric_chm_cpd_static_nov24.RDS"))
  
  post_ric_chm_cpd_static=ric_chm_cpd_static$draws(variables=c('b_for','b_for_cu','b_for_rv',
                                                           # 'b_npgo','b_npgo_cu','b_npgo_rv',
                                                           'alpha_j','Smax','sigma'),format='draws_matrix')
  write.csv(post_ric_chm_cpd_static,here('stan models','outs','posterior','ric_chm_cpd_static_nov24.csv'))
