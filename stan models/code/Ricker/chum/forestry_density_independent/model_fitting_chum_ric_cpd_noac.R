...
dl_chm_eca=list(N=nrow(ch20r),
                ...
                pRk_mean=smax_prior$m.r, ##prior for Rk (recruitment capacity) based on max observed spawners
...
if(Sys.info()[7] == "mariakur") {
  ...
  ric_chm_eca <- mric$sample(data=dl_chm_eca,
                            ...
                            adapt_delta = 0.99,
                            max_treedepth = 20)
  write.csv(ric_chm_eca$summary(),'./stan models/outs/summary/ric_chm_eca_noac_trial.csv')
  ric_chm_eca$save_object('./stan models/outs/fits/ric_chm_eca_noac_trial.RDS')

  post_ric_chm_eca=ric_chm_eca$draws(variables=c('b_for','b_for_cu','b_for_rv','alpha_t','alpha_j','Smax','sigma'),format='draws_matrix')
  write.csv(post_ric_chm_eca,here('stan models','outs','posterior','ric_chm_eca_noac_trial.csv'))
  ...
  write.csv(ric_chm_eca$summary(),here("stan models","outs","summary","ric_chm_eca_noac.csv"))
  ric_chm_eca$save_object(here("stan models","outs","fits","ric_chm_eca_noac.RDS"))

  post_ric_chm_eca=ric_chm_eca$draws(variables=c('b_for','b_for_cu','b_for_rv','alpha_t','alpha_j','Smax','sigma'),format='draws_matrix')
  write.csv(post_ric_chm_eca,here('stan models','outs','posterior','ric_chm_eca_noac.csv'))
...
if(Sys.info()[7] == "mariakur") {
  ...
  ric_chm_cpd <- mric$sample(data=dl_chm_cpd,
                            ...
                            adapt_delta = 0.99,
                            max_treedepth = 20)
  write.csv(ric_chm_cpd$summary(),'./stan models/outs/summary/ric_chm_cpd_noac_trial.csv')
  ric_chm_cpd$save_object('./stan models/outs/fits/ric_chm_cpd_noac_trial.RDS')
  
  post_ric_chm_cpd=ric_chm_cpd$draws(variables=c('b_for','b_for_cu','b_for_rv','alpha_t','alpha_j','Smax','sigma',
                                                 'sigma_for_cu','sigma_for_rv'),format='draws_matrix')
  write.csv(post_ric_chm_cpd,here('stan models','outs','posterior','ric_chm_cpd_noac_trial.csv'))
  ...
  write.csv(ric_chm_cpd$summary(),here("stan models","outs","summary","ric_chm_cpd_noac.csv"))
  ric_chm_cpd$save_object(here("stan models","outs","fits","ric_chm_cpd_noac.RDS"))
  
  post_ric_chm_cpd=ric_chm_cpd$draws(variables=c('b_for','b_for_cu','b_for_rv','alpha_t','alpha_j','Smax','sigma',
                                                 'sigma_for_cu','sigma_for_rv'),format='draws_matrix')
  write.csv(post_ric_chm_cpd,here('stan models','outs','posterior','ric_chm_cpd_noac.csv'))
