...
transformed parameters{
  ...
  vector[N] K_for; //time varying K with forestry
  ...
  for(j in 1:J){//for every stock
    ...
    K_for[start_y[j]] = K[j] + b_for_rv[j]*forest_loss[start_y[j]]; //adjust K based on forest loss, npgo, sst
    ...
    for(t in (start_y[j]+1):(end_y[j])){ //adjust expectation based on autocorrelation
      ...
      K_for[t] = K[j] + b_for_rv[j]*forest_loss[t]; //adjust K based on forest loss
      b[t] = 1/K_for[t];
