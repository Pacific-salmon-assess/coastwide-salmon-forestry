...
file_ric=file.path(here('stan models', 'code','Ricker','chum','forestry_K', 'ric_chm_npgo_sst_K.stan'))
...
if(Sys.info()[7] == "mariakur") {
  ...
  ric_chm_eca <- mric$sample(data=dl_chm_eca,
                             ...
                             adapt_delta = 0.95,
                             max_treedepth = 20)
  write.csv(ric_chm_eca$summary(),'./stan models/outs/summary/ric_chm_eca_npgo_sst_K_trial.csv')
  ric_chm_eca$save_object('./stan models/outs/fits/ric_chm_eca_npgo_sst_K_trial.RDS')
  ...
  write.csv(post_ric_chm_eca,here('stan models','outs','posterior','ric_chm_eca_npgo_sst_K_trial.csv'))
  ...
  ric_chm_eca$save_object(here("stan models","outs","fits","ric_chm_eca_npgo_sst_K.RDS"))
  
  post_ric_chm_eca=ric_chm_eca$draws(variables=c('b_for','b_for_cu','b_for_rv',
                                                 'b_npgo','b_npgo_cu','b_npgo_rv',
                                                 'b_sst','b_sst_cu','b_sst_rv',
                                                 'alpha_j','K','sigma'),format='draws_matrix')
  write.csv(post_ric_chm_eca,here('stan models','outs','posterior','ric_chm_eca_npgo_sst_K.csv'))
...
if(Sys.info()[7] == "mariakur") {
  ...
  ric_chm_cpd <- mric$sample(data=dl_chm_cpd,
                             ...
                             adapt_delta = 0.95,
                             max_treedepth = 20)
  write.csv(ric_chm_cpd$summary(),'./stan models/outs/summary/ric_chm_cpd_npgo_sst_K_trial.csv')
  ric_chm_cpd$save_object('./stan models/outs/fits/ric_chm_cpd_npgo_sst_K_trial.RDS')
  
  post_ric_chm_cpd=ric_chm_cpd$draws(variables=c('b_for','b_for_cu','b_for_rv',
                                                 'b_npgo','b_npgo_cu','b_npgo_rv',
                                                 'b_sst','b_sst_cu','b_sst_rv',
                                                 'alpha_j','K','sigma'),format='draws_matrix')
  write.csv(post_ric_chm_cpd,here('stan models','outs','posterior','ric_chm_cpd_npgo_sst_K_trial.csv'))
  ...
  ric_chm_cpd$save_object(here("stan models","outs","fits","ric_chm_cpd_npgo_sst_K.RDS"))
  
  post_ric_chm_cpd=ric_chm_cpd$draws(variables=c('b_for','b_for_cu','b_for_rv',
                                                 'b_npgo','b_npgo_cu','b_npgo_rv',
                                                 'b_sst','b_sst_cu','b_sst_rv',
                                                 'alpha_j','K','sigma'),format='draws_matrix')
  write.csv(post_ric_chm_cpd,here('stan models','outs','posterior','ric_chm_cpd_npgo_sst_K.csv'))
