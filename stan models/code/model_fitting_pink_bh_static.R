...
file_bh_st=file.path(here('stan models', 'code', 'bh_pink_static_mk.stan'))
...
if(Sys.info()[7] == "mariakur") {
  ...
  bh_pk_eca_st <- mbh_p_st$sample(data=dl_pk_eca,
                                      ...
                                      adapt_delta = 0.999,
                                      max_treedepth = 20)
  write.csv(bh_pk_eca_st$summary(),here("stan models","outs","summary","bh_pk_eca_st_trial.csv"))
  bh_pk_eca_st$save_object(here("stan models","outs","fits","bh_pk_eca_st_trial.RDS"))
  
  post_bh_pk_eca_st=bh_pk_eca_st$draws(variables=c('b_for','b_for_cu','b_for_rv','alpha_j','Rk','sigma'),format='draws_matrix')
  write.csv(post_bh_pk_eca_st,here('stan models','outs','posterior','bh_pk_eca_st_trial.csv'))
