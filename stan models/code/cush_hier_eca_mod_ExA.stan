...
transformed parameters{
  ...
  //residual productivity deviations
   for(j in 1:J){ //for every stock
	mu1[start_y[j]:end_y[j]]=alpha_j[j] +b[j]*log(S[start_y[j]:end_y[j],j]) +b_ECA_cu[C_i[j]]*ECA[start_y[j]:end_y[j],j]+b_Area_cu[C_i[j]]*Area[start_y[j]:end_y[j],j]+b_ECA_Area_cu[C_i[j]]*ExA[start_y[j]:end_y[j],j]; //expectation (unadjusted for autocorrelated residuals)
