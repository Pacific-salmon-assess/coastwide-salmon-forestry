...
//residual productivity deviations
   ...
   b[j]=1/Smax[j];
    mu1[start_y[j]]=alpha_j[j]-b[j]*S[start_y[j]]+b_for_rv[j]*forest_loss[start_y[j]] + b_npgo_rv[j]*npgo[start_y[j]]; //adjust expectation based on previous deviate - rho is raised to the power of the number of time steps (in years) between observations
    ...
    for(t in (start_y[j]+1):(end_y[j])){ //adjust expectation based on autocorrelation
      mu2[t]  = alpha_j[j]-b[j]*S[t]+b_for_rv[j]*forest_loss[t]+ b_npgo_rv[j]*npgo[t]+ rho[j]^(ii[t]-ii[t-1])*e_t[t-1]; //adjust expectation based on previous deviate - rho is raised to the power of the number of time steps (in years) between observations
