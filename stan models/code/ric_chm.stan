...
//BH eq. recruitment
  vector[J] log_b; //log per capita density dependence - Ricker beta parameter
...
transformed parameters{
  ...
  vector<lower=0>[J] b; // Ricker beta parameter
...
//residual productivity deviations
   for(j in 1:J){ //for every stock
	for(t in (start_y[j]):(end_y[j])){ //adjust expectation based on autocorrelation
	  mu[t] = alpha_t[ii[t]]+alpha_j[j]-b[j]*S[t]+b_for_rv[j]*forest_loss[t]; //adjust expectation based on previous deviate - rho is raised to the power of the number of time steps (in years) between observations
      e_t[t] = R_S[t] - mu[t];  //residual for stock j at time t
	}
  }
 }  
model{
  //priors
  //productivity
   alpha_0 ~ normal(1.5,2.5); //global intrinsic productivity for all stocks at time t=1;
  ...
  //Smax for each stock - fit individually with weakly informative priors based on maximum observed spawners
