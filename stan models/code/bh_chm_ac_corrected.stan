...
//residual productivity deviations
   for(j in 1:J){ //for every stock
    alpha_j_for[start_y[j]] = alpha_j[j]+b_for_rv[j]*forest_loss[start_y[j]];
    mu1[start_y[j]]=alpha_t[start_t[j]]+alpha_j_for[start_y[j]]-log(1+(exp(alpha_t[start_t[j]]+alpha_j_for[start_y[j]])/Rk[j])*S[start_y[j]]); //adjust expectation based on previous deviate - rho is raised to the power of the number of time steps (in years) between observations
    ...
    for(t in (start_y[j]+1):(end_y[j])){ //adjust expectation based on autocorrelation
      ...
      mu2[t]  = alpha_t[ii[t]]+alpha_j_for[t]-log(1+(exp(alpha_t[ii[t]]+alpha_j_for[t])/Rk[j])*S[t]) + rho[j]^(ii[t]-ii[t-1])*e_t[t-1]; //adjust expectation based on previous deviate - rho is raised to the power of the number of time steps (in years) between observations
