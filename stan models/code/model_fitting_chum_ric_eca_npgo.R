...
file_ric=file.path(here('stan models', 'code', 'ric_chm_static_npgo.stan'))
...
if(Sys.info()[7] == "mariakur") {
  print("Running on local machine")
  ric_chm_eca_npgo <- mric$sample(data=dl_chm_eca_npgo,
                            chains = 1, 
                            iter_warmup = 20,
                            iter_sampling =50,
                            refresh = 10,
                            adapt_delta = 0.999,
                            max_treedepth = 20)
  write.csv(ric_chm_eca_npgo$summary(),'./stan models/outs/summary/ric_chm_eca_npgo_trial.csv')
  ric_chm_eca_npgo$save_object('./stan models/outs/fits/ric_chm_eca_npgo_trial.RDS')
  
  post_ric_chm_eca_npgo=ric_chm_eca_npgo$draws(variables=c('b_for','b_for_cu','b_for_rv',
                                                           'b_npgo','b_npgo_cu','b_npgo_rv',
                                                           'alpha_j','Smax','sigma'),format='draws_matrix')
  write.csv(post_ric_chm_eca_npgo,here('stan models','outs','posterior','ric_chm_eca_npgo_trial.csv'))
  ...
  write.csv(ric_chm_eca_npgo$summary(),here("stan models","outs","summary","ric_chm_eca_npgo.csv"))
  ric_chm_eca_npgo$save_object(here("stan models","outs","fits","ric_chm_eca_npgo.RDS"))
  
  post_ric_chm_eca_npgo=ric_chm_eca_npgo$draws(variables=c('b_for','b_for_cu','b_for_rv',
                                                           'b_npgo','b_npgo_cu','b_npgo_rv',
                                                           'alpha_j','Smax','sigma'),format='draws_matrix')
  write.csv(post_ric_chm_eca_npgo,here('stan models','outs','posterior','ric_chm_eca_npgo.csv'))
...
if(Sys.info()[7] == "mariakur") {
  print("Running on local machine")
  ric_chm_cpd_npgo <- mric$sample(data=dl_chm_cpd_npgo,
                                  chains = 1, 
                                  iter_warmup = 20,
                                  iter_sampling =50,
                                  refresh = 10,
                                  adapt_delta = 0.999,
                                  max_treedepth = 20)
  write.csv(ric_chm_cpd_npgo$summary(),'./stan models/outs/summary/ric_chm_cpd_npgo_trial.csv')
  ric_chm_cpd_npgo$save_object('./stan models/outs/fits/ric_chm_cpd_npgo_trial.RDS')
  
  post_ric_chm_cpd_npgo=ric_chm_cpd_npgo$draws(variables=c('b_for','b_for_cu','b_for_rv',
                                                           'b_npgo','b_npgo_cu','b_npgo_rv',
                                                           'alpha_j','Smax','sigma'),format='draws_matrix')
  write.csv(post_ric_chm_cpd_npgo,here('stan models','outs','posterior','ric_chm_cpd_npgo_trial.csv'))
  ...
  write.csv(ric_chm_cpd_npgo$summary(),here("stan models","outs","summary","ric_chm_cpd_npgo.csv"))
  ric_chm_cpd_npgo$save_object(here("stan models","outs","fits","ric_chm_cpd_npgo.RDS"))
  
  post_ric_chm_cpd_npgo=ric_chm_cpd_npgo$draws(variables=c('b_for','b_for_cu','b_for_rv',
                                                           'b_npgo','b_npgo_cu','b_npgo_rv',
                                                           'alpha_j','Smax','sigma'),format='draws_matrix')
  write.csv(post_ric_chm_cpd_npgo,here('stan models','outs','posterior','ric_chm_cpd_npgo.csv'))
