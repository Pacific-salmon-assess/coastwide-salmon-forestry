...
//residual productivity deviations
   for(j in 1:J){ //for every stock
     	mu1[start_y[j]]=alpha_t[start_t[j]]+alpha_j[j]-log(1+(exp(alpha_t[start_t[j]]+alpha_j[j])/Rk[j])*S[start_y[j]])+b_for_rv[j]*forest_loss[start_y[j]]; //expectation (unadjusted for autocorrelated residuals)
