...
file_bh=file.path(here('stan models', 'code', 'bh_pink_noac.stan'))
...
if(Sys.info()[7] == "mariakur") {
  ...
  bh_pk_eca <- mbh_p$sample(data=dl_pk_eca,
                            ...
                            adapt_delta = 0.999,
                            max_treedepth = 20)
  write.csv(bh_pk_eca$summary(),'./stan models/outs/summary/bh_pk_eca_noac_trial.csv')
  ...
  write.csv(post_bh_pk_eca,here('stan models','outs','posterior','bh_pk_eca_noac_trial.csv'))
  ...
  tryCatch(
        ...
        {
        #some expression
          write.csv(bh_pk_eca$summary(),here("stan models","outs","summary","bh_pk_eca_noac.csv"))
          bh_pk_eca$save_object(here("stan models","outs","fits","bh_pk_eca_noac.RDS"))
  
          post_bh_pk_eca=bh_pk_eca$draws(variables=c('b_for','b_for_cu','b_for_rv','alpha_t','alpha_j','Rk'),format='draws_matrix')
          write.csv(post_bh_pk_eca,here('stan models','outs','posterior','bh_pk_eca_noac.csv'))
        },
...
if(Sys.info()[7] == "mariakur") {
  ...
  write.csv(bh_pk_cpd$summary(),'./stan models/outs/summary/bh_pk_cpd_noac_trial.csv')
  bh_pk_cpd$save_object('./stan models/outs/fits/bh_pk_cpd_noac_trial.RDS')
  
  post_bh_pk_cpd=bh_pk_cpd$draws(variables=c('b_for','b_for_cu','b_for_rv','alpha_t','alpha_j','Rk'),format='draws_matrix')
  write.csv(post_bh_pk_cpd,here('stan models','outs','posterior','bh_pk_cpd_noac_trial.csv'))
  ...
  tryCatch(
        ...
        {
        #some expression
          write.csv(bh_pk_cpd$summary(),here("stan models","outs","summary","bh_pk_cpd_noac.csv"))
          bh_pk_cpd$save_object(here("stan models","outs","fits","bh_pk_cpd_noac.RDS"))
  
          post_bh_pk_cpd=bh_pk_cpd$draws(variables=c('b_for','b_for_cu','b_for_rv','alpha_t','alpha_j','Rk'),format='draws_matrix')
          write.csv(post_bh_pk_cpd,here('stan models','outs','posterior','bh_pk_cpd_noac.csv'))
        },
        #if an error occurs, tell me the error
        error=function(e) {
            message('An Error Occurred')
