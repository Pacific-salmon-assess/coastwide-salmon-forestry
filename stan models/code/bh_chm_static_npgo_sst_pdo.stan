data{
  ...
  array[N] real pdo; //vector of pdo through time
...
//covariate effects
 ...
 real b_pdo; //global (across stock) mean effect of pdo
 ...
 vector[C] z_pdo_cu; //CU-specific pdo z-score deviation
 vector[J] z_pdo_rv; //River-specific pdo z-score deviation
 ...
 real<lower=0> sigma_pdo_cu; //variance in CU-level ECA effect
 real<lower=0> sigma_pdo_rv; //variance in river-level ECA effect
...
vector[C] b_pdo_cu; //CU-specific pdo effect
vector[J] b_pdo_rv; //River-specific pdo effect
...
b_pdo_cu = b_pdo + sigma_pdo_cu*z_pdo_cu; //non-centered CU-varying estimate for pdo effects
b_pdo_rv = b_pdo_cu[C_i] + sigma_pdo_rv*z_pdo_rv; //non-centered CU-varying estimate for pdo effects
...
//residual productivity deviations
   for(j in 1:J){ //for every stock
    mu1[start_y[j]]=alpha_j[j]-log(1+(exp(alpha_j[j])/Rk[j])*S[start_y[j]])+b_for_rv[j]*forest_loss[start_y[j]] + b_npgo_rv[j]*npgo[start_y[j]] + b_sst_rv[j]*sst[start_y[j]]+ b_pdo_rv[j]*pdo[start_y[j]]; //adjust expectation based on previous deviate - rho is raised to the power of the number of time steps (in years) between observations
    ...
    for(t in (start_y[j]+1):(end_y[j])){ //adjust expectation based on autocorrelation
      mu2[t]  = alpha_j[j]-log(1+(exp(alpha_j[j])/Rk[j])*S[t])+b_for_rv[j]*forest_loss[t]+ b_npgo_rv[j]*npgo[t]+ b_sst_rv[j]*sst[t] + b_pdo_rv[j]*pdo[t] +rho[j]^(ii[t]-ii[t-1])*e_t[t-1]; //adjust expectation based on previous deviate - rho is raised to the power of the number of time steps (in years) between observations
...
model{
 ...
 //covariate effects
  ...
  b_pdo ~ normal(0,1); //standard normal prior for the effect of pdo
  ...
  z_pdo_cu ~ std_normal(); //std normal prior for CU-deviations
  z_pdo_rv ~ std_normal(); //std normal prior for River-deviations
 ...
 sigma_for_cu ~ normal(0,1); //variance in stock-level forest loss effects
  ...
  sigma_pdo_cu ~ normal(0,1); //variance in stock-level pdo effects
  sigma_pdo_rv ~ normal(0,1); //variance in stock-level pdo effects
