...
parameters{
  ...
  vector[J] alpha; //initial global productivity
...
transformed parameters{
  ...
  mu1[start_y[j]]=alpha[j]-log(1+(exp(alpha[j])/Rk[j])*S[start_y[j]])+b_for_rv[j]*forest_loss[start_y[j]]; //adjust expectation based on previous deviate - rho is raised to the power of the number of time steps (in years) between observations
  ...
  mu2[t]  = alpha[j]-log(1+(exp(alpha[j])/Rk[j])*S[t])+b_for_rv[j]*forest_loss[t]+ rho[j]^(ii[t]-ii[t-1])*e_t[t-1]; //adjust expectation based on previous deviate - rho is raised to the power of the number of time steps (in years) between observations
...
model{
  //priors
  //productivity
  alpha ~ normal(1.5,2); //global intrinsic productivity for all stocks at time t=1;
  // for(j in 1:J) alpha_j[j] ~ normal(1.5,2);
