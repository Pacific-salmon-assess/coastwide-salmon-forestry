...
//residual productivity deviations
   ...
   alpha_j_for[start_y[j]] = alpha_j[j]+b_for_rv[R_i[j]]*forest_loss[start_y[j]]; //initial expectation for stock j
   mu1[start_y[j]]=alpha_t[BL[j],start_t[j]]+alpha_j_for[start_y[j]]-log(1+(exp(alpha_t[BL[j],start_t[j]]+alpha_j_for[start_y[j]])/Rk[j])*S[start_y[j]]);
   e_t[start_y[j]] = R_S[start_y[j]] - mu1[start_y[j]]; //first deviate for stock j
	...
	for(t in (start_y[j]+1):(end_y[j])){ //adjust expectation based on autocorrelation
	  alpha_j_for[t] = alpha_j[j]+b_for_rv[R_i[j]]*forest_loss[t]; //expectation for stock j at time t
	  mu2[t]  = alpha_t[BL[j],ii[t]]+alpha_j_for[t]-log(1+(exp(alpha_t[BL[j],ii[t]]+alpha_j_for[t])/Rk[j])*S[t]);
