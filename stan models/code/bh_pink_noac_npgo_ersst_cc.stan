...
transformed parameters{
  ...
  vector<lower=0>[N] Rk_for; //recruitment capacity adjusted for forest loss
  vector[J] log_Rk; //log-transformed recruitment capacity
  vector[N] log_Rk_for; //log-transformed recruitment capacity adjusted for forest loss
...
vector[N] alpha_j_for; //productivity with forest loss effect
...
//residual productivity deviations
   ...
   alpha_j_for[start_y[j]] = alpha_j[j]+ b_npgo_rv[R_i[j]]*npgo[start_y[j]]+ b_sst_rv[R_i[j]]*sst[start_y[j]]; //forestry effects on just productivity
   log_Rk_for[start_y[j]]=log(Rk[j])+b_for_rv[R_i[j]]*forest_loss[start_y[j]]; //adjust recruitment capacity based on forest loss
   Rk_for[start_y[j]]=exp(log_Rk_for[start_y[j]]); //convert back to linear scale
   mu1[start_y[j]]=alpha_j_for[start_y[j]]-log(1+(exp(alpha_j_for[start_y[j]])/Rk_for[start_y[j]])*S[start_y[j]]);
   e_t[start_y[j]] = R_S[start_y[j]] - mu1[start_y[j]]; //first deviate for stock j
	...
	for(t in (start_y[j]+1):(end_y[j])){ //adjust expectation based on autocorrelation
	  log_Rk_for[t]=log_Rk[j]+b_for_rv[R_i[j]]*forest_loss[t]; //adjust recruitment capacity based on forest loss
    Rk_for[t]=exp(log_Rk_for[t]); //convert back to linear scale
	  alpha_j_for[t] = alpha_j[j]+ b_npgo_rv[R_i[j]]*npgo[t]+ b_sst_rv[R_i[j]]*sst[t]; //forestry effects on just productivity
	  mu2[t]  = alpha_j_for[t] -log(1+(exp(alpha_j_for[t])/Rk_for[t])*S[t]);
