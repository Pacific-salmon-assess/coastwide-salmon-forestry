...
The first set of models includes ECA alone (logit transformed and standardized) with varying effects down to the level individual rivers (subscript r) in either the Ricker, Beverton-Holt, or Cushing structure (see m1_ricker.stan, m1_bh.stan, m1_cush.stan). The second set drops the varying effects for ECA at the River scale (but varying effects for the CU scale remain). The third set includes watershed area and its interaction with the ECA effects.
...
Stock-specific density-dependence is estimated independently at the river level, but with informative priors for $S_{max}$ (=$1/\beta$) based on observed spawner counts ($S_r$):
...
Where the new parameter $R_{k,r}$ is the stock-specific equibrium recruitment, sampled with informative priors based on observed recruitment ($R$):
